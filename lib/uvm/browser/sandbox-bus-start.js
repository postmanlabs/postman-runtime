...
(function (win) {
    var MessageBus,
        ...
        // Holds all the Javascript global objects, so that user provided values are not overridden
        jsGlobals = Object.keys(win);
    ...
    Scope = function (replacements) {
        ...
        // Always copy the cached array, because we later mutate it, plus each execution should be independent
        globals = jsGlobals.slice(0);
    ...
    Scope.allowedGlobals = [
        ...
        // For compatibility with the old Chrome App
        'window', 'XMLHttpRequest',

        'console', 'btoa', 'atob', 'moment' // (temporary)
    ];
    ...
    um.bus = new MessageBus(win.parent, {
        ...
        // todo: uncomment when we start adding restrictions and breaking eval/window.* workflows
