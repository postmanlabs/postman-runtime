...
(function (win) {
    ...
    um.PostmanApi = function (masked, options) {
        ...
        for (prop in options.responseHeaders) {
            options.responseHeaders.hasOwnProperty(key) &&
            (insensitiveHeaders[key.toLowerCase()] = options.responseHeaders[key])
        }

        for (prop in options.responseCookies) {
            if (options.responseCookies.hasOwnProperty(prop)) {
                (insensitiveCookies[prop.toLowerCase()] = options.responseCookies[prop]);
            }
        }
