...
module.exports = {
    ...
    pre: function (auth, done) {
        ...
        done(null, Boolean(params.nonce && params.realm));
    },
    ...
    post: function (auth, response, done) {
        ...
        if ((code === 401 || code === 403) && authHeader) {
            ...
            authVarList.syncFromObject({nonce: nonce, realm: realm}, false, false); // no tracking & no pruning required
    ...
    sign: function (auth, request, done) {
        ...
        if (!params ||
            !params.username ||
            !params.realm) { done(); } // Nothing to do if no parameters are present.
