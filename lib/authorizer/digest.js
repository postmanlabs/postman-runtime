...
/**
 * `realm` and `nonce` should be stored and reused for Digest auth. If not found or if the the server responds with 401,
 * a second request should be sent using the values sent by the server.
 * Also the previously stored `realm` and `nonce` should be discarded and new values should be stored.
 *
...
module.exports = {
     ...
     */
    pre: function (auth, state, done) {
        // ensure that all dynamic parameter values are present in the parameters
        // if even one is absent, we return false.
        done(null, (Boolean(state.nonce) && Boolean(state.realm)) || (Boolean(auth.get('nonce') && auth.get('realm'))));
    },
    ...
    post: function (auth, response, done) {
        ...
        if ((code === 401 || code === 403) && authHeader) {
            ...
            // test for both user set value and server returned value
            if (auth.get(QOP) || authParams.qop) {
            ...
            return done(null, false, authParams);
        }
     ...
     */
    sign: function (auth, request, state, done) {
        var header,
            params = _.merge(auth.get(AUTH_PARAMETERS), state);
