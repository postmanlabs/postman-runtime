const controlCommand = require('../runner/extensions/control.command');
...
var _ = require('lodash'),
    ...
    EMPTY = '',
    NONCE_COUNT_PADDED_LENGTH = 8,
    NONCE_COUNT_PADDING_STRING = '0',
    HEXADECIMAL_BASE = 16,
    DEFAULT_NONCE_COUNT = (1).toString(HEXADECIMAL_BASE)
        .padStart(NONCE_COUNT_PADDED_LENGTH, NONCE_COUNT_PADDING_STRING),
...
/**
 * Returns an incremented nonceCount
 *
 * @param {String} nonceCount - hexadecimal value
 * @param {Integer} step - step count to increase, defaults to 1
 * @private
 */
function _incrementNonceCount (nonceCount, step) {
    !step && (step = 1);

    return (parseInt(nonceCount, HEXADECIMAL_BASE) + step).toString(HEXADECIMAL_BASE)
        .padStart(NONCE_COUNT_PADDED_LENGTH, NONCE_COUNT_PADDING_STRING);
}
...
module.exports = {
    ...
    sign: function (auth, request, done) {
        ...
        if (params.qop) {
            ...
            auth.set('nonceCount', nonceCount ? _incrementNonceCount(nonceCount) : DEFAULT_NONCE_COUNT);
        }
