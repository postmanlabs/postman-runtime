...
Authorizer = function (options, callback) {
    ...
    // If we not are given an object, treat it as a boolean, and assign to all auth types.
    if (!_.isObject(options && options.interactive)) {
        this.interactive = _.transform(Authorizer.Handlers, function (accumulator, handler, name) {
            accumulator[name] = Boolean(options.interactive);
        }, {});
    } else {
    ...
    this.interactive = !_.isObject(options && options.interactive) ?
...
_.assign(Authorizer.prototype, /** @lends Authorizer.prototype */ {
    ...
    preVerify: function (context, run, callback) {
        ...
        Requester.create({
            ...
            source: _.get(handler, AUTH_TYPE_PROP, PRE + '.auth')
    ...
    init: function (context, run, callback) {
        ...
        Requester.create({
            type: 'http',  // @todo - how can the auth declare what type it needs?
            source: _.get(handler, AUTH_TYPE_PROP, INIT + '.auth')
        }, run.options.requester, function (err, requester) {
            if (err) { return callback(err); }
    ...
    postVerify: function (context, run, callback) {
        ...
        Requester.create({
            type: 'http',  // @todo - how can the auth declare what type it needs?
            source: _.get(handler, AUTH_TYPE_PROP, POST + '.auth')
        }, run.options.requester, function (err, requester) {
            if (err) { return callback(err); }
