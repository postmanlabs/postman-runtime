var _ = require('lodash'),
    ...
    GLOBAL_MAX_REPLAY_COUNT = 10,
    DEFAULT_MAX_REPLAY_COUNT = 1,
...
_.assign(Authorizer, /** @lends Authorizer */ {
    /**
     * Limit for number of times a request can be replayed.
     */
    GLOBAL_MAX_REPLAY_COUNT: GLOBAL_MAX_REPLAY_COUNT,

    /**
     * Default max value for number of times a request can be replayed.
     */
    DEFAULT_MAX_REPLAY_COUNT: DEFAULT_MAX_REPLAY_COUNT,
    ...
    addHandler: function (Handler, name) {
        ...
        if (_.isInteger(Handler.maxReplayCount)) {
            if (Handler.maxReplayCount > GLOBAL_MAX_REPLAY_COUNT || Handler.maxReplayCount < 0) {
                throw new Error('The handler for "' + name + '" has an invalid "maxReplayCount" value.' +
                    ' Value must be between 0-' + GLOBAL_MAX_REPLAY_COUNT + ' inclusive.');
            }
        }
        else {
            Handler.maxReplayCount = DEFAULT_MAX_REPLAY_COUNT;
        }
