...
module.exports = {
    ...
    pre: function (auth, done) {
        var state = context.auth.parameters().toObject().state,
            authVarList = auth[auth.type];

        !state && authVarList.syncFromObject({state: STATES.INITIALIZED}, false, false);
    ...
    post: function (auth, response, done) {
        var authVarList = auth[context.auth.type],
            state = authVarList.toObject().state,
            typeOneMessage,
            typeTwoMessage,
            typeThreeMessage,
            item = context.item;
        ...
        if (state === STATES.INITIALIZED) {
            ...
            // Update the state
            state = STATES.T1_MSG_CREATED;
            authVarList.syncFromObject({state: state}, false, false);
