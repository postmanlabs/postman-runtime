...
module.exports = {
    ...
    post: function (auth, response, done) {
        ...
        if (response.code !== 401 && response.code !== 403) {
            console.log('successful', response.code);
        ...
        if (state === STATES.INITIALIZED) {
            ...
            auth.set('ntlmHeader', typeOneMessage);
            ...
            typeTwoMessage = ntlmUtil.parseType2Message(response.headers.get('www-authenticate') || '', _.noop);
            ...
            auth.set('ntlmHeader', typeThreeMessage);
    ...
    sign: function (auth, request, done) {
        ...
        if (auth.get('ntlmHeader')) {
            request.addHeader({
                key: 'Authorization',
                value: auth.get('ntlmHeader'),
                system: true
            });
        }
        return done();
