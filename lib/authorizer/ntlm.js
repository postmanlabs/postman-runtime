...
module.exports = {
    ...
    init: function (item, run, cursor, done) {
        this._ = this._ || {};
        this._.state = STATES.INITIALIZED;
        done(null);
    },
    ...
    post: function (item, response, run, cursor, done) {
        ...
        if (auth._.state === STATES.INITIALIZED) {
            ...
            // Update the state
            auth._.state = STATES.T1_MSG_CREATED;

            // ask runtime to replay the request
            return done(null, false);
        }
        else if (auth._.state === STATES.T1_MSG_CREATED) {
            ...
            auth._.state = STATES.T3_MSG_CREATED;

            // ask runtime to replay the request
            return done(null, false);
        }
        else if (auth._.state === STATES.T3_MSG_CREATED) {
