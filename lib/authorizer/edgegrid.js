...
var _ = require('lodash'),
    ...
    getTimestamp = function () {
        ...
        timestamp = date.getUTCFullYear() +
                    String(date.getUTCMonth() + 1).padStart(2, ZERO) +
                    String(date.getUTCDate()).padStart(2, ZERO) +
                    DATE_TIME_SEPARATOR +
                    String(date.getUTCHours()).padStart(2, ZERO) +
                    COLON +
                    String(date.getUTCMinutes()).padStart(2, ZERO) +
                    COLON +
                    String(date.getUTCSeconds()).padStart(2, ZERO) +
                    UTC_OFFSET;
    ...
    canonicalizeHeaders = function (headersToSign, headers) {
        ...
        headersToSign.forEach(function (headerName) {
            ...
            formattedHeaders.push(`${headerName.toLowerCase()}:${headerValue.trim().replace(/\s+/g, SPACE)}`);
        });
...
module.exports = {
    ...
    computeHeader: function (params) {
        ...
        dataToSign = [
            params.method,
            params.url.protocol.replace(COLON, EMPTY),
