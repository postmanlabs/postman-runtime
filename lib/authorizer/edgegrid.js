...
var _ = require('lodash'),
     ...
     * @param {Integer} maxBody Maximum body size to hash
     */
    filterBody = function (body, maxBody) {
        if (!body) { return EMPTY; }
        // As per Akamai documentation, they employ this logic to calculate the body hash:
        // for non-ASCII values, they treat each as 1 byte. Therefore, we must convert them
        // into an array before calculating the length of the string
        // https://github.com/akamai/AkamaiOPEN-edgegrid-python/blob/master/akamai/edgegrid/edgegrid.py#L81
        let trimmedBody = Array.from(body.toString());

        if (body.length > maxBody) {
            trimmedBody = trimmedBody.slice(0, maxBody);
        }

        return trimmedBody.join('') || EMPTY;
    },
     ...
     */
    computeBodyHash = function (body, algorithm, digestEncoding, maxBody, callback) {
        ...
        if (body.mode === RequestBody.MODES.raw) {
            ...
            hash.update(filterBody(rawBody, maxBody));
        ...
        if (body.mode === RequestBody.MODES.urlencoded) {
            ...
            hash.update(filterBody(urlencodedBody, maxBody));
        ...
        if (body.mode === RequestBody.MODES.file) {
            ...
            return originalReadStream.cloneReadStream(function (err, clonedStream) {
                ...
                clonedStream.on('data', function (chunk) {
                    hash.update(filterBody(chunk, maxBody));
                });
        ...
        if (body.mode === RequestBody.MODES.graphql) {
            ...
            hash.update(filterBody(graphqlBody, maxBody));
...
module.exports = {
    ...
    sign: function (auth, request, done) {
        ...
        params.maxBody = params.maxBody || 131072; // 128 KB
        ...
        if (request.method === 'POST') {
            return computeBodyHash(request.body, 'sha256', 'base64', params.maxBody, function (bodyHash) {
