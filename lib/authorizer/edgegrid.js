...
var _ = require('lodash'),
     ...
     */
    resizeBodyToMaxLimit = function (body, maxBody) {
        if (!body || maxBody <= 0) { return EMPTY; }
    ...
    computeBodyHash = function (body, algorithm, digestEncoding, maxBody, callback) {
        ...
        if (body.mode === RequestBody.MODES.raw) {
            ...
            hash.update(resizeBodyToMaxLimit(rawBody, maxBody));
        ...
        if (body.mode === RequestBody.MODES.urlencoded) {
            ...
            hash.update(resizeBodyToMaxLimit(urlencodedBody, maxBody));
        ...
        if (body.mode === RequestBody.MODES.file) {
            ...
            return originalReadStream.cloneReadStream(function (err, clonedStream) {
                ...
                let remainingMaxBody = maxBody;

                clonedStream.on('data', function (chunk) {
                    hash.update(resizeBodyToMaxLimit(chunk, remainingMaxBody));
                    remainingMaxBody -= chunk.byteLength;
                });
        ...
        if (body.mode === RequestBody.MODES.graphql) {
            ...
            hash.update(resizeBodyToMaxLimit(graphqlBody, maxBody));
