...
module.exports = {
    ...
    computeBodyHash: function (body, algorithm, digestEncoding, fileResolver, callback) {
        if (!body || !algorithm) { return callback(); }
        ...
        if (body.mode === RequestBody.MODES.urlencoded) {
            hash.update(this.rfc3986(querystring.stringify(urlencodedBodyBuilder(body.urlencoded).form)));
        ...
        if (body.mode === RequestBody.MODES.file) {
            ...
            util.createReadStream(fileResolver, _.get(body, 'file.src'), function (err, stream) {
    ...
    sign: function (auth, request, done) {
        ...
        self.computeBodyHash(request.body, 'sha256', 'hex', auth.fileResolver, function (bodyHash) {
