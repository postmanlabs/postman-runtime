...
var minimumHeadersToSign = ['x-date', '(request-target)', 'host'],
    ...
    getBodyString = function (body) {
        ...
        console.log('unable to get body string');
        throw new Error('Unable to get body string!');
    },
    ...
    processRequestForOci = function (ociRequest, auth) {
    // Host header
        ...
        console.log('-----> before body if ');
        if (ociRequest.body && _.includes(methodsThatRequireExtraHeaders, ociRequest.method)) {
            if (!ociRequest.headers.has(HEADER_CONTENT_TYPE)) {
                ociRequest.headers.set(HEADER_CONTENT_TYPE, 'application/json');
            }
            let contentLen = 0;
            const shaObj = new jssha('SHA-256', 'TEXT'),
                requestBodyString = getBodyString(ociRequest.body);

            if (requestBodyString) {
                shaObj.update(requestBodyString);
                ociRequest.headers.set(HEADER_CONTENT_SHA, shaObj.getHash('B64'));
                contentLen = Buffer.byteLength(requestBodyString, 'utf8');
            }
            if (contentLen === 0) {
                // if buffer is empty, it can only be an empty string payload
                ociRequest.headers.set(HEADER_CONTENT_SHA, EMPTY_SHA);
            }
            if (!ociRequest.headers.has(HEADER_CONTENT_LEN)) {
                ociRequest.headers.set(HEADER_CONTENT_LEN, `${contentLen}`);
            }
            headersToSign = headersToSign.concat(HEADER_CONTENT_TYPE, HEADER_CONTENT_LEN, HEADER_CONTENT_SHA);
        }
        else {
            console.log('NO REQUEST BODY LOL');
        }
        ...
        console.log('Before httpSignature.sign', ociRequest.headers);
        ...
        // NOTE: OCI needs 'authorization' but our version of httpSignature puts signature in 'Authorizaiton'
        ...
        ociRequest.headers.delete('Authorization');


        console.log('result: ', authorizationHeader);
        console.log('allHeaders', ociRequest.headers);
...
module.exports = {
    ...
    sign: function (auth, request, done) {
        ...
        console.log(signedOciRequest.body);
        request.addHeader({
            key: 'authorization',
        ...
        if (signedOciRequest.headers.get(HEADER_CONTENT_LEN)) {
            request.addHeader({
                key: HEADER_CONTENT_LEN,
                value: signedOciRequest.headers.get(HEADER_CONTENT_LEN),
                system: true
            });
        }
        if (signedOciRequest.headers.get(HEADER_CONTENT_SHA)) {
            request.addHeader({
                key: HEADER_CONTENT_SHA,
                value: signedOciRequest.headers.get(HEADER_CONTENT_SHA),
                system: true
            });
        }
        if (signedOciRequest.headers.get(HEADER_CONTENT_TYPE)) {
            request.addHeader({
                key: HEADER_CONTENT_TYPE,
                value: signedOciRequest.headers.get(HEADER_CONTENT_TYPE),
                system: true
            });
        }
