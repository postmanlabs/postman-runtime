...
module.exports = {
    ...
    sign: function (auth, request, done) {
        ...
        if (isHsAlgorithm && !_.isString(secret)) {
            return done(new Error('jwt: secret must be a string'));
        }

        // bail out - private key not a valid string
        if (!isHsAlgorithm && !_.isString(privateKey)) {
            return done(new Error('jwt: private key must be a string'));
        }
