...
function addTokenToRequest (auth, request, jwtToken) {
    ...
    if (addTokenTo === ADD_TOKEN_TO_TARGETS.HEADER) {
        ...
        if (_.isNil(headerPrefix)) {
            // null or undefined add Bearer prefix as default
            request.addHeader({
                key: AUTHORIZATION,
                value: BEARER_AUTH_PREFIX + SPACE + jwtToken,
                system: true
            });
        }
        else {
            // empty headerPrefix - add token alone
            // else add headerPrefix with token
            request.addHeader({
                key: AUTHORIZATION,
                value: _.isEmpty(headerPrefix) ? jwtToken : headerPrefix + SPACE + jwtToken,
                system: true
            });
        }
    }
