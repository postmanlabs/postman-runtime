...
function addTokenToRequest (auth, request, jwtToken) {
    ...
    if (addTokenTo === ADD_TOKEN_TO_TARGETS.HEADER) {
        ...
        request.addHeader({
            key: AUTHORIZATION,
            value: headerPrefix ? (headerPrefix + SPACE + jwtToken) : jwtToken,
            system: true
        });
    }
 ...
 *                             If headerPrefix is any truthy value - `Authorization:headerPrefix + SPACE + JWT_TOKEN`
 *                             If headerPrefix is any falsy  value - `Authorization:JWT_TOKEN`
