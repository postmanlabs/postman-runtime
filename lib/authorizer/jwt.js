...
module.exports = {
    ...
    sign: function (auth, request, done) {
        ...
        try {
            ...
            // must stringify payload - else by 'iat claim' will be auto add in payload
            const payload = JSON.stringify(auth.get(PAYLOAD)),
                options = {
                    algorithm: auth.get(ALGORITHM),
                    header: JSON.parse(auth.get(HEADER)) // header must be valid object
                };
