...
_.extend(Cursor.prototype, {
    ...
    reset: function (length, cycles, position, iteration, ref, callback, scope) {
        ...
        isNil(length) && (length = this.length);
        isNil(cycles) && (cycles = this.cycles);
        isNil(position) && (position = this.position);
        isNil(iteration) && (iteration = this.iteration);
        isNil(ref) && (ref = this.ref);
    ...
    whatnext: function (coords) {
        ...
        if (!_.isObject(coords)) {
            return _.extend(base, {eof: true, bof: true, empty: this.empty()});
        }
        if (!this.length) {
            return _.extend(base, {eof: true, bof: true, empty: true});
        }
        ...
        return _.extend(base, {position: position, iteration: iteration});
    },
