...
_.extend(Cursor.prototype, {
    ...
    reset: function (length, cycles, position, iteration, ref, callback, scope) {
        ...
        (cycles == null) && (cycles = this.cycles);
        (position == null) && (position = this.position);
        (iteration == null) && (iteration = this.iteration);
        (ref == null) && (ref = this.ref);
    ...
    whatnext: function (coords) {
        if (!_.isObject(coords)) { return { eof: true, bof: true, empty: this.empty(), ref: this.ref }; }
        if (!this.length) { return { eof: true, bof: true, empty: true, ref: this.ref }; }
        ...
        if (position >= this.length) {
            ...
            if (iteration >= this.cycles) {
                return { position: this.length - 1, iteration: iteration - 1, eof: true, ref: this.ref };
            }

            return { position: position, iteration: iteration, cr: true, ref: this.ref };
        }

        return { position: position, iteration: iteration, length: this.length, cycles: this.cycles, ref: this.ref };
    },
