var sdk = require('postman-collection'),
     ...
     * @returns {Object}
     */
    findFlowItemsOrGroups = function (itemGroup, entrypointSubset, _continueAccumulation, _accumulatedItems) {
        if (!itemGroup || !itemGroup.items) { return; }
        ...
        itemGroup.items.each(function (item) {
            ...
            if (match) {
                // only accumulate items which are not previously got tracked from its parent entrypoint.
                _continueAccumulation && (_accumulatedItems[match] = item);
            ...
            // recursive call to find nested entry-points. To make sure all provided entry-points got tracked.
            // _continueAccumulation flag will be `false` for children if their parent entrypoint is found.
            return findFlowItemsOrGroups(item, entrypointSubset, !match, _accumulatedItems);
        });
    ...
    lookupByFlow = function (collections, options, callback) {
        ...
        collections.forEach(function (collection) {
            items = Object.assign(items, findFlowItemsOrGroups(collection, entrypointLookup, true));
        });
        ...
        entrypoints.forEach(function (entrypoint) {
            runnableItems = runnableItems.concat(flattenNode(items[entrypoint]));
        });

        callback(null, runnableItems, collections);
    },
