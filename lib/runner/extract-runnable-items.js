var sdk = require('postman-collection'),
    ...
    // error codes that runtime throws
    ERR_INVALID_LOOKUP_STRATEGY = 1,
    ERR_INVALID_ENTRY_POINTS = 2,
    ERR_INVALID_FOLDER_OR_REQUEST = 3,
    INVALID_LOOKUP_STRATEGY_ERROR = 'runtime~extractRunnableItems: Invalid entrypoint lookupStrategy',
    INVALID_ENTRYPOINTS_ERROR = 'runtime~extractRunnableItems: Invalid entrypoints',
    ...
    createError = function ({ errorMessage, errorCode }) {
    ...
    lookupByPath = function (collection, options, callback) {
        ...
        return callback(createError({ errorMessage: INVALID_ENTRYPOINTS_ERROR,
            errorCode: ERR_INVALID_ENTRY_POINTS }), []);
    },
    ...
    lookupByIdOrName = function (collection, options, callback) {
        ...
        const folderOrRequestName = _.get(options, 'execute');

        return callback(createError({ errorMessage: `Unable to find a folder or request: ${folderOrRequestName}`,
            errorCode: ERR_INVALID_FOLDER_OR_REQUEST }), [], match);
    },
    ...
    extractRunnableItems = function (collection, entrypoint, callback) {
        ...
        (lookupFunction = lookupStrategyMap[lookupStrategy]) ?
            ...
            callback(createError({
                errorMessage: INVALID_LOOKUP_STRATEGY_ERROR,
                errorCode: ERR_INVALID_LOOKUP_STRATEGY
            }));
    };
