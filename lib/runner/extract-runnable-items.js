var sdk = require('postman-collection'),
    ...
    lookupByMultipleIdOrName = function (collection, options, callback) {
        ...
        // Sorting the items and folders by the order in which they appear as folder/request arguments,
        // only if specified in the entrypoint.preserveOrder
        preserveOrder && items && items.members && _.isArray(items.members) && items.members.sort(function (a, b) {
            // Getting the keys of the folder/request as lookups can be on the basis of either id or name.
            let key1 = _.includes(entrypoints, a.name) ? entrypoints.indexOf(a.name) : entrypoints.indexOf(a.id),
                key2 = _.includes(entrypoints, b.name) ? entrypoints.indexOf(b.name) : entrypoints.indexOf(b.id);
     ...
     * @param {Boolean} [entrypoint.preserveOrder] Value to preserve the order of folders/items or not
     * @param {Function} callback
     */
