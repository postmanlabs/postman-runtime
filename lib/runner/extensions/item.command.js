var _ = require('lodash'),
    uuid = require('uuid'),
    extractNextRequestInstruction = require('../set-next-request').extractNextRequestInstruction;
                ...
                this.queue('event', {
                    ...
                    this.queue('request', {
                        ...
                        this.queue('event', {
                            ...
                            extractNextRequestInstruction(((stopOnError || stopOnFailure) && testExecutionError) ?
                                testExecutionError : null, {
                                prerequest: prereqExecutions,
                                request: request,
                                response: response,
                                test: testExecutions
                            }, this, coords, function (err, nextPayload, jump) {
                                // trigger an event saying that item has been processed
                                // @todo - should this trigger receive error?
                                this.triggers.item(null, coords, item, jump);
                                callback && callback(err, nextPayload);
                            }.bind(this));
                        });
                    });
                });
