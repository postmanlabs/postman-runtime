var _ = require('lodash'),
    ...
    resolveFiles = function (request, resolver, triggers, callback) {
        var mode = _.get(request, 'body.mode'),
            formParams = _.get(request, 'body.formdata'),
            fileData = _.get(request, 'body.file');

        if (!mode) { return callback(null, request); }

        if (mode === 'formdata') {
            return async.reduce(formParams.all(), [], function (list, param, cb) {
            ...
            request.body.file = {
                content: fileData.src && resolver.createReadStream(fileData.src)
            };
            return callback(null, request);
        }
        else {
