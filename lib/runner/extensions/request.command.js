var _ = require('lodash'),
    ...
    { generateKeyDomainMatchPatternsMap, getMatchingVariables } = require('../domains'),
    ...
    resolveVariables = function (context, payload) {
        ...
        var variableDefinitions = [
            ...
            vaultSecrets = _.cloneDeep(payload.vaultSecrets.values),
            urlString = context.item.request.url.toString(),
            item = context.item,
            auth = context.auth,
            keyDomainMatchPatternsMap,
            matchingVaultSecrets = [];

        if (vaultSecrets) {
            // 1. Traverse all the vault variables and have a set of unique domain pattern
            keyDomainMatchPatternsMap = generateKeyDomainMatchPatternsMap(vaultSecrets);

            matchingVaultSecrets = getMatchingVariables(keyDomainMatchPatternsMap, urlString, vaultSecrets);
        }
        ...
        if (urlString) {
            ...
            urlString = sdk.Property.replaceSubstitutions(urlString, [
                ...variableDefinitions,
                new VariableScope(matchingVaultSecrets).values
            ]);
