const { hasDomain, hasMatchingDomain } = require('../domains');
...
var _ = require('lodash'),
    ...
    resolveVariables = function (context, payload) {
        ...
        var variableDefinitions = [
            ...
            vaultSecrets = payload.vaultSecrets.toJSON().values,
            urlString = context.item.request.url.toString(),
            item = context.item,
            auth = context.auth,
            emptyDomainVaultVariables = new VariableScope(vaultSecrets.filter((v) => { return !hasDomain(v); }));
        ...
        if (urlString) {
            ...
            urlString = sdk.Property.replaceSubstitutions(urlString, [
                ...variableDefinitions,
                emptyDomainVaultVariables.values
            ]);
        ...
        const validVaultVariables = new VariableScope(vaultSecrets.filter(hasMatchingDomain(urlString)));

        variableDefinitions = [
            ...variableDefinitions,
            validVaultVariables.values
        ];
