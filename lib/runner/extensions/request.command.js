var _ = require('lodash'),
    ...
    createContext = function (payload) {
        var context = payload.context ? payload.context : _.pick(payload, [
                'data',
                'environment',
                'globals',
                'coords'
    ...
    process: {
        ...
        request: function (payload, next) {
            ...
            async.applyEachSeries(prehelpers, context, self, function (err) {
                ...
                // @note -  we give the _original_ item in this trigger, so someone can do reference checking.
                //          Not sure if we should do that or not, but that's how it is. Don't break it.
                self.triggers.beforeRequest(null, context.cursor, item.request, payload.item, {
                    abort: function () {
