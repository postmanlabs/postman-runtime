    ...
    process: {
        ...
        request: function (payload, next) {
            ...
            async.applyEachSeries(prehelpers, payload, self, function (err) {
                ...
                xhr = self.requester.request(item, function (err, response, request, _res, _req) {
                    err = err || null;
                    ...
                    var nextPayload = {
                        ...
                        sdkResponse = new sdk.Response(response);

                    // Unconditionally trigger the request event.
                    self.triggers.request(err, payload.coords, sdkResponse,
                        item.request, payload.item,
                        // todo: remove these asap
                        _res, _req);

                    if (err && abortOnError) {
                        // the error is passed twice to allow control between
                        return next(err, nextPayload, err);
                    }

                    item.responses.add(sdkResponse);

                    // run the post request helpers
