    ...
    process: {
        ...
        waterfall: function (payload, next) {
            ...
            this.queue('event', {
                ...
                // sync results with our state & context
                // @todo forward setnext request to test script
                if (result && result.globals) {
                    result.globals.environment &&
                        (ctxTemplate.environment = util.syncObject(this.state.environment, result.globals.environment));
                    result.globals.globals &&
                        (ctxTemplate.globals = util.syncObject(this.state.globals, result.globals.globals));
                }
                ...
                this.queue('request', {
                    ...
                    this.queue('event', {
                        ...
                        ctx: _.assign(_.cloneDeep(ctxTemplate), {
                            request: request,
                            tests: {},
                        ...
                        if (result.globals) {
                            // sync results with our state
                            result.globals.environment &&
                                util.syncObject(this.state.environment, result.globals.environment);
                            result.globals.globals &&
                                util.syncObject(this.state.globals, result.globals.globals);
                        }
