...
module.exports = {
    init: function (done) {
        ...
        // a streaming iteration-data source is not an array — leave it intact
        if (!state.data || typeof state.data.getIterationRow !== 'function') {
    ...
    process: {
        ...
        waterfall (payload, next) {
            ...
            function queueIteration () {
                self.queueDelay(function () {
                    this.queue('item', {
                        item: item,
                        coords: coords,
                        data: getIterationData(this.state.data, iterationIndex),
            ...
            // Streaming iteration data: ensure this iteration's row is buffered
            // before the synchronous getIterationData read. No-op for arrays.
            if (data && typeof data.ensure === 'function') {
                return data.ensure(iterationIndex).then(queueIteration).catch(next);
            }
