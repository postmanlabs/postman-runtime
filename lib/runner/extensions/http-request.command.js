...
module.exports = {
    ...
    process: {
        ...
        httprequest: function (payload, next) {
            ...
            /**
             *  points to request currently being processed so that SshTunnelAgent can route the request to
             *  correct host in case multiple ssh hosts are being configured.
             */
            this.currentReq = payload.item.request.url.toString();
            ...
            async.applyEachSeries(prehelpers, context, self, function (err) {
                ...
                self.requester.create({
                    ...
                    xhr = requester.request(sendId, item.request, context.protocolProfileBehavior, function (err, res, req, cookies) {
                        err = err || null;
