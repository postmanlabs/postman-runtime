var _ = require('lodash'),
     ...
     * @param {Function} callback function invoked with error, sanitizedRequest and affetedEntries.
     *                            affectedEntries is the list of files removed from request.
     */
    sanitizeFiles = function (request, callback) {
        ...
        if (request.body.mode === REQUEST_BODY_MODE_FILE) {
            ...
            sanitizedRequest = _.cloneDeep(request);
            sanitizedRequest.body.file = null;

            // add file to affectedEntries
            affectedEntries.push(_.get(request, 'body.file.src'));

            // send sanitizedRequest with file body removed
            return callback(null, sanitizedRequest, affectedEntries);
        }

        // handle request body in `formdata` mode
        if (request.body.mode === REQUEST_BODY_MODE_FORMDATA) {
            // extract any file type params
            _.forEach(request.body.formdata, function (formParam) {
                if (formParam.type === FILE) {
                    // collect file paths
                    affectedEntries.push(formParam.src);
                }
                else {
                    // only push params that are not of type file
                    sanitizedFormdata.push(formParam);
                }
            });
            ...
            if (affectedEntries.length > 0) {
                ...
                sanitizedRequest = _.cloneDeep(request);
                sanitizedRequest[REQUEST_BODY_MODE_FORMDATA] = sanitizedFormdata;

                // send request with file params removed from formdata
                return callback(null, sanitizedRequest, affectedEntries);
            }

            // if none of the formdata params is a file type, nothing was changed
            return callback(null, request);
        }

        // for any other request body mode no sanitizing needed
        return callback(null, request);
    };
        ...
        event: function (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_REQUEST_EVENT_BASE + executionId, function (cursor, id, requestId, request) {
                    ...
                    sanitizeFiles(request, function (err, request, affectedEntries) {
                        if (err) {
                            return this.host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + id, requestId, err);
                        }

                        var cursorWithSource = _.clone(cursor), // clone cursor and add source
                            tempItem;

                        // if request is sanitized send a warning
                        if (!_.isEmpty(affectedEntries)) {
