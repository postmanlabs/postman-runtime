        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_VAULT_BASE + executionId, function (id, cmd, ...args) {
                    ...
                    if (!allowVaultAccess) {
                        return dispatch(new Error('Vault access denied'));
                    }

                    if (!['get', 'set', 'unset'].includes(cmd)) {
                        return dispatch(new Error(`Invalid vault command: ${cmd}`));
                    }
                ...
                const currentEventItem = event.parent && event.parent(),
                    ...
                    executeScript = ({ resolvedPackages }, done) => {
                        ...
                        this.host.execute(event, {
                            ...
                            // vault secrets are not sent to sandbox,
                            // thus using the scope from run context.
                            allowVaultAccess && vaultSecrets &&
