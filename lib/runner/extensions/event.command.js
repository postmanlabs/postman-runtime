        ...
        event: function (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                    ...
                    function (eventId, action, fnName, args) {
                        ...
                        if (!cookieStore) {
                            return self.host.dispatch(dispatchEvent, eventId,
                                new Error('CookieStore: no store found'));
                        }

                        if (typeof cookieStore[fnName] !== 'function') {
                            return self.host.dispatch(dispatchEvent, eventId,
                                new Error(`CookieStore: invalid method name '${fnName}'`));
                        }
                ...
                this.host.execute(event, {
                    ...
                    visualiserResults = result && result.return && result.return.visualiser;

                    if (visualiserResults) {
                        Visualiser.processTemplate(visualiserResults.template,
                            visualiserResults.data,
                            visualiserResults.options,
                            function (err, processedTemplate) {
                                if (err) {
                                    // bubble up the errors while processing template through visualiser results
                                    visualiserResults.error = err;

                                    return;
                                }

                                // add processed template and data to visualiser results
                                visualiserResults.processedTemplate = processedTemplate;
                                visualiserResults.globals = result.globals && result.globals.toObject();
                                visualiserResults.environment = result.environment && result.environment.toObject();

                                try {
                                    visualiserResults.response = result.response && result.response.json();
                                }
                                catch (err) {
                                    // don't set response if it is not a valid JSON response
                                }
                            });
                    }
