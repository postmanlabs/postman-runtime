var _ = require('lodash'),
    ...
    sanitizeFiles = function (request, callback) {
        ...
        if (!request.body) {
            // empty sanitizedRequest implies nothing needs to be changed
        ...
        if (request.body.mode === REQUEST_BODY_MODE_FILE) {
            ...
            // send request with file body removed
            return callback(null, sanitizedRequest);
        }
        ...
        event: function (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_REQUEST_EVENT_BASE + executionId, function (cursor, id, requestId, request) {
                    ...
                    sanitizeFiles(request, function (err, sanitizedRequest) {
                        // @todo - there is no error flows now
                        // this is here to allow extending sanitizeFiles
                        // to include error flows future
                        if (err) {
                            return;
                        }

                        var tempRequest = request,
                            cursorWithSource = _.clone(cursor), // clone cursor and add source
                            tempItem;

                        // if request is sanitized and files are removed send a warning
                        if (sanitizedRequest) {
                            this.triggers.console(cursor, 'warn', 'uploading files from scripts is not allowed');
                            tempRequest = sanitizedRequest;
                        }
