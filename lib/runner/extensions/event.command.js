...
module.exports = {
    ...
    process: {
         ...
         *
         * @param {Function=} failures - A function that extracts failures from a tests object.
         ...
         * @param {String} [payload.trackFailuresIn]
         ...
         */
        event: function (failures, payload, next) {
            // adjust for polymorphic instructions
            if (!next && _.isFunction(payload) && !_.isFunction(failures)) {
                next = payload;
                payload = failures;
                failures = null;
            }
            ...
            var suppressEventPropagation = _.has(this.options, 'suppressEventPropagation') ?
                ...
                trackFailuresIn = _.isString(payload.trackFailuresIn) && payload.trackFailuresIn,
                abortOnFailure = _.has(payload, 'abortOnFailure') ?
                    payload.abortOnFailure : this.options.abortOnFailure,
                stopOnScriptError = _.has(payload, 'stopOnScriptError') ? payload.stopOnScriptError :
                    this.options.stopOnScriptError,
                // abortOnFailure implies abortOnError
                abortOnError = abortOnFailure || (_.has(payload, 'abortOnError') ?
                    payload.abortOnError : this.options.abortOnError),
                events;
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.execute(script.toSource(), {
                    ...
                    // Get the failures if abortOnFailure is set
                    // If there was an error running the script itself, that takes precedence
                    if (abortOnFailure && (!err) && _.isFunction(failures)) {
                        err = failures(_.get(result.globals, trackFailuresIn));
                    }
                    ...
                    next(((stopOnScriptError || abortOnError) && err) ? err : null, _.assign({
                        event: event,
                        script: script,
                        result: result
