            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                    function (scriptCursor, id, requestId, requestToRunId) {
                        ...
                        self.triggers.console(scriptCursor, 'info', 'executing nested collection request');
                        ...
                        collectionRequestResolver(requestToRunId, (err, resolvedRequestCollection) => {
                            if (err || !resolvedRequestCollection) {
                                // Stop execution of the entire request
                                return self.host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + id, requestId, err);
                            }
                            ...
                            new collectionRequestRunner().run(resolvedRequestCollection, { abortOnFailure: true },
                                function (err, run) {
                                    ...
                                    run.start({
                                        ...
                                        // TODO: How do we handle for things like Server Sent Events in nested requests?
