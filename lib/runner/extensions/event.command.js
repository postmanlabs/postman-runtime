        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                (abortOnError || stopOnScriptError) &&
                    ...
                    this.host.once(EXECUTION_ERROR_EVENT_BASE + executionId, function (scriptCursor, error) {
                        if (error && !(error instanceof Error)) {
                            // TODO: For connection errors and stuff, serialize the error received here from nested
                            // executions
                ...
                this.host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                    function (scriptCursor, eventId, requestId, requestToRunId, nestedRequestOptions) {
                        const self = this,
                            containerRequestPassedOptions = self.state;
                        ...
                        function dispatchErrorToListener (err) {
                            ...
                            return self.host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + eventId, requestId, error);
                        }
                        ...
                        if (!containerRequestPassedOptions.requestResolverBridge) {
                            // TODO: This could be running in the postman cli, so
                            // do we throw an error while this functionality isn't implemented
                            // or do we error out and stop execution.
                            // Because this will currently stop execution.
                            return dispatchErrorToListener(new Error('pm.execution.runRequest(...) is not supported'));
                        }
                        ...
                        containerRequestPassedOptions.requestResolverBridge(requestToRunId,
                            function (err, normalizedRequestItemToExecute) {
                                if (err) {
                                    return dispatchErrorToListener(err);
                                }

                                if (!normalizedRequestItemToExecute) {
                                    return dispatchErrorToListener(new Error('Expected a full normalized request item' +
                                ...
                                const collectionRequestRunner = require('../../runner'),
                                    nestedReqLocalVariables = nestedRequestOptions.variables ?
                                        Object.entries(nestedRequestOptions.variables)
                                            .map(([key, value]) => {
                                                return {
                                                    key: key,
                                                    value: value,
                                        ...
                                        [],
                                    mergedLocalVariableValues = [
                                        ...(containerRequestPassedOptions.localVariables ?
                                            containerRequestPassedOptions.localVariables.values || [] : []),
                                        ...(nestedReqLocalVariables)
                                    ],
                                    runnableCollection = new sdk.Collection({
                                        info: {
                                            _postman_id: uuid.v4(),
                                            name: 'Placeholder Collection with single request for nested execution',
                                            schema: 'https://schema.getpostman.com/json/collection/v2.1.0' +
                                                '/collection.json'
                                        },
                                        item: [normalizedRequestItemToExecute],
                                        // TODO: Refactor this entire block to accept collection json with variable
                                        // definition from the resolver bridge
                                        variable: containerRequestPassedOptions.collectionVariables &&
                                            containerRequestPassedOptions.collectionVariables.values ?
                                            containerRequestPassedOptions.collectionVariables.values
                                ...
                                new collectionRequestRunner().run(runnableCollection,
                                    {
                                        ...containerRequestPassedOptions,
                                        currentRequestNestingDepth: (self.state.currentRequestNestingDepth || 0) + 1,
                                        localVariables: { values: mergedLocalVariableValues },
                                        abortOnFailure: true,
                                    ...
                                    function (err, run) {
                                        ...
                                        if (err) {
                                            return self.host
                                                .dispatch(EXECUTION_RESPONSE_EVENT_BASE + eventId, requestId, err);
                                        }
                                        run.start({
                                            script (_err, _cursor, result) {
                                                // Sync mutations/changes to pm.variables, pm.environment
                                                // and pm.globals to parent scripts.
                                                ...
                                                const mutationsListFromThisScriptExecutions = {
                                                    _variables: result._variables.mutations,
                                                    environment: result.environment.mutations,
                                                    globals: result.globals.mutations
                                                };

                                                self.host.dispatch(EXECUTION_SYNC_VARIABLES_FROM_ANOTHER_EXECUTION +
                                                    executionId, mutationsListFromThisScriptExecutions);

                                                // With this approach, we don't need to invoke the script listener
                                                // of the parent request, since we propagate mutations, the final list
                                                // will include all mutations when the parent request's script listener
                                                // is invoked by postman-runtime.
                                            },
                                            exception (_, err) {
                                                if (err) {
                                            ...
                                            response (err, _, response) {
                                                ...
                                                return self.host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + eventId,
                                                    requestId, err || exceptionForThisRequest || null,
                ...
                this.host.on(EXECUTION_SKIP_REQUEST_EVENT_BASE + executionId, function () {
                    ...
                    if (self.options.isNestedRequest && self.triggers.response) {
                        self.triggers.response(null, null, null);
                    }
                });
