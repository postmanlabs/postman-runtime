...
module.exports = {
    ...
    process: {
         ...
         * @param {Array.<String>} [payload.trackFailuresIn]
        ...
        event: function (payload, next) {
            var suppressEventPropagation = _.has(this.options, 'suppressEventPropagation') ?
                ...
                abortOnFailure = _.has(payload, 'abortOnFailure') ? payload.abortOnFailure : this.options.abortOnFailure,
            ...
            util.asyncEach(events, function (event, next) {
                ...
                this.host.execute(script.toSource(), {
                    ...
                    // Track failures
                    abortOnFailure && !err && (err = (function (tests) {
                        var message,
                            t;
                        for (t in tests) {
                            if (tests.hasOwnProperty(t) && !tests[t]) {
                                message = message || '';
                                message += (t + ' failed\n');
                            }
                        }
                        if (message) {
                            return new serialisedError(Error(message), true);
                        }
                    }(_.get(result, 'globals.' + trackFailuresIn))));
