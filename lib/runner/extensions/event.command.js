    ...
    process: {
        ...
        event (payload, next) {
            var item = payload.item,
                ...
                packagesResolver = this.options.packagesResolver,
            ...
            async.mapSeries(events, function (event, next) {
                ...
                const currentEventItem = event.parent && event.parent(),
                    ...
                    executeScript = ({ resolvedPackages }) => {
                ...
                if (!packagesResolver) {
                    return executeScript({ resolvedPackages: {} });
                }

                packagesResolver({ packages: script.packages }, (err, resolvedPackages) => {
                    if (err || !resolvedPackages) {
                        return next(err);
                    }
