var _ = require('lodash'),
    ...
    SandboxPool = require('../sandbox-pool'),
...
module.exports = {
    init: function (done) {
        ...
        // Attaches the run-level (not per-execution) listeners to a context.
        // These fire for whichever context ran a script; they route by the
        // `execution` id carried in the event payload, so they are safe to
        // attach identically to every context in the pool.
        const attachRunListeners = function (context) {
            ...
            // Number of sandbox contexts (uvm worker threads) to run scripts on.
            // Default 1 preserves the historical single-host behaviour. Values
            // > 1 let CPU-bound scripts across parallel iterations (VUs) run on
            // multiple cores. Bounded to a sane floor of 1.
            poolSize = Math.max(1, _.get(run, 'options.sandboxPoolSize', 1) | 0),

            createOptions = _.merge({
        ...
        // No pool is created here — a single, externally-owned host is used
        // directly via `run.host` (see acquireHost fallback below).
        if (run.host) {
            return done();
        }
        ...
        // Default (poolSize === 1): keep the historical single-host behaviour
        // exactly — one context on `run.host`, no pool object, no acquire/release
        // machinery. This guarantees zero behavioural change unless a caller
        // explicitly opts in with sandboxPoolSize > 1.
        if (poolSize <= 1) {
            return sandbox.createContext(createOptions, function (err, context) {
                if (err) { return done(err); }

                attachRunListeners(context);
                run.host = context;
        ...
        // poolSize > 1: create `poolSize` contexts up-front; each is an
        // independent uvm worker thread so CPU-bound scripts across parallel
        // iterations (VUs) can run on multiple cores.
        async.times(poolSize, function (n, next) {
            sandbox.createContext(_.merge({}, createOptions), function (err, context) {
            ...
            // keep `run.host` pointing at the first context for backward compat
            // (external references, nested-request) and expose the full pool
            // via `run.hostPool`.
            run.host = contexts[0];
            run.hostPool = new SandboxPool(contexts);
    ...
    process: {
        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                // Nested requests (pm.execution.runRequest) reuse the parent
                // execution's context — acquiring a fresh one from the pool while
                // the parent still holds one could deadlock at small pool sizes.
                // Top-level scripts acquire a context from the pool so that
                // parallel iterations (VUs) execute on different worker threads.
                // `this.hostPool` may be absent when a caller sets `this.host`
                // directly and bypasses `init` (e.g. unit tests, external hosts);
                // fall back to the single host in that case.
                var isNestedRequest = this.state.nestedRequest !== undefined,
                    acquireHost = (isNestedRequest || !this.hostPool) ?
                        (cb) => { return cb(this.host); } :
                        (cb) => { return this.hostPool.acquire(cb); },
                    releaseHost = (isNestedRequest || !this.hostPool) ?
                        function () { /* parent execution / single host owns the context */ } :
                        (host) => { return this.hostPool.release(host); };

                return acquireHost((host) => {
                // get access to the script from the event.
                    ...
                    // add event listener to trap all assertion events, but only if needed. to avoid needlessly accum
                    // stuff in memory.
                    (abortOnFailure || stopOnFailure) &&
                    host.on(EXECUTION_ASSERTION_EVENT_BASE + executionId, function (scriptCursor, assertions) {
                    ...
                    host.once(EXECUTION_ERROR_EVENT_BASE + executionId, function (scriptCursor, error) {
                    ...
                    host.on(EXECUTION_COOKIES_EVENT_BASE + executionId,
                        ...
                        // more cookie actions in next 2 years ¯\_(ツ)_/¯
                            ...
                            if (!cookieStore) {
                                return host.dispatch(dispatchEvent, eventId, 'CookieStore: no store found');
                            }

                            if (typeof cookieStore[fnName] !== 'function') {
                                return host.dispatch(dispatchEvent, eventId,
                            ...
                            // serialize error message
                                ...
                                host.dispatch(dispatchEvent, eventId, err, res);
                            });
                            ...
                            try {
                                ...
                                host.dispatch(dispatchEvent, eventId,
                    ...
                    // This is important especially when dealing with nested requests, without this, the parent might
                    ...
                    host.on(EXECUTION_VAULT_BASE + executionId, async function (id, cmd, ...args) {
                        ...
                        const dispatch = (e, r) => { host.dispatch(EXECUTION_VAULT_BASE + executionId, id, e, r); };
                    ...
                    host.on(EXECUTION_DATASETS_BASE + executionId, function (id, cmd, datasetId, ...args) {
                        const datasetsResolver = _.get(this, 'options.script.datasetsResolver'),

                            // Ensure error is string
                            // TODO identify why error objects are not being serialized correctly
                            dispatch = (e, r) => {
                                host.dispatch(EXECUTION_DATASETS_BASE + executionId, id, e, r);
                            };
                    ...
                    host.on(EXECUTION_REQUEST_EVENT_BASE + executionId,
                        ...
                        // remove files in request body if any
                            sanitizeFiles(request, function (err, request, sanitisedFiles) {
                                if (err) {
                                    return host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + id, requestId, err);
                                }
                                ...
                                this.immediate('httprequest', nextPayload).done(function (result) {
                                    host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + id,
                                        requestId,
                                        null,
                                    ...
                                    host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + id, requestId, error);
                                });
                            }.bind(this));
                        }.bind(this));

                    host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                        runNestedRequest({ executionId, isExecutionSkipped, vaultSecrets, item }).bind(this));

                    host.on(EXECUTION_SKIP_REQUEST_EVENT_BASE + executionId, function () {
                    ...
                    const currentEventItem = event.parent && event.parent(),
                        ...
                        executeScript = ({ resolvedPackages }, done) => {
                            ...
                            host.execute(event, {
                                ...
                                host.removeAllListeners(EXECUTION_REQUEST_EVENT_BASE + executionId);
                                host.removeAllListeners(EXECUTION_ASSERTION_EVENT_BASE + executionId);
                                host.removeAllListeners(EXECUTION_RESPONSE_EVENT_BASE + executionId);
                                host.removeAllListeners(EXECUTION_RUN_REQUEST_RESPONSE_EVENT_BASE + executionId);
                                host.removeAllListeners(EXECUTION_COOKIES_EVENT_BASE + executionId);
                                host.removeAllListeners(EXECUTION_ERROR_EVENT_BASE + executionId);
                                host.removeAllListeners(EXECUTION_SKIP_REQUEST_EVENT_BASE + executionId);
                                host.removeAllListeners(EXECUTION_VAULT_BASE + executionId);
                                host.removeAllListeners(EXECUTION_DATASETS_BASE + executionId);

                                // Script is done with this context (all mid-execution
                                // dispatches have fired); return it to the pool so a
                                // waiting iteration can run on this worker thread.
                                releaseHost(host);
                                ...
                                // if certain variables are to be synced back to result, we do the same
                                ...
                                // Get the failures. An error running the script itself takes precedence
                                ...
                                // persist the pm.variables for the next request in the current partition
                                    this.partitionManager
                                        .updatePartitionVariables(payload.coords.partitionIndex, result);
                                }
                                ...
                                // this script is done executing, trigger the event and move to the next script
                                ...
                                done(((stopOnScriptError || abortOnError || stopOnFailure) && err) ?
                                    err : null, _.assign({
                                    event,
                                    script,
                                    result
                    ...
                    return packageResolver({ packages: script.packages }, (err, resolvedPackages) => {
                        if (err) {
                        // release the context we acquired but never executed on
                            releaseHost(host);
