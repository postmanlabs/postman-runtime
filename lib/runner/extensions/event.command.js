...
//     fs.appendFileSync('logfile.txt', logs.map((item) => {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                    function (scriptCursor, id, requestId, requestToRunId) {
                        ...
                        collectionRequestResolver(requestToRunId,
                            self.state.requestResolverBridge, (err, resolvedRequestCollection) => {
                                ...
                                new collectionRequestRunner().run(resolvedRequestCollection, { abortOnFailure: true },
                                    function (err, run) {
                                        if (err) {
                                            return self.host
                                        ...
                                        run.start({
                                            ...
                                            // TODO: Handle for things like Server Sent Events in nested requests
