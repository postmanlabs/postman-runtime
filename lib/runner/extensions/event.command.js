        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_DATASETS_BASE + executionId, function (id, cmd, datasetId, ...args) {
                    ...
                    // For __pull/__cancel the streamId arrives in the datasetId slot.
                    if (cmd === '__pull') {
                        const entry = datasetStreams.get(datasetId);
                        ...
                        return pullDatasetBatch(entry.iterator, DATASET_STREAM_BATCH).then((frame) => {
                            if (frame.done) { datasetStreams.delete(datasetId); }

                            dispatch(null, frame);
                        }).catch((err) => {
                            datasetStreams.delete(datasetId);
