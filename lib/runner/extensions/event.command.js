var _ = require('lodash'),
    ...
    EXECUTION_REQUIRE_EVENT_BASE = 'execution.require.',
    EXECUTION_REQUIRE_RESPONSE_EVENT_BASE = 'execution.requireResponse.',
    ...
    process: {
        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_REQUIRE_EVENT_BASE + executionId,
                    function (id, requestId, name) {
                        if (!fileResolver || typeof fileResolver.readFile !== 'function') {
                            return this.host.dispatch(EXECUTION_REQUIRE_RESPONSE_EVENT_BASE + id,
                                requestId,
                                new Error('FileResolver not found'));
                        }

                        fileResolver.readFile(name, 'utf8', function (err, content) {
                            this.host.dispatch(EXECUTION_REQUIRE_RESPONSE_EVENT_BASE + id,
                                requestId,
                                err,
                                content);
                        }.bind(this));
