    ...
    process: {
        ...
        event: function (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                (stopOnScriptError || abortOnError) &&
                    this.host.on(EXECUTION_ERROR_EVENT_BASE + executionId, function (scriptCursor, err) {
                        // only store first async error in case of multiple errors
                        !asyncScriptError && (asyncScriptError = err);
                    });
                ...
                this.host.execute(event, {
                    ...
                    // Handle errors from async script. If there was an error running the script itself, that takes precedence
                    if (!err) {
