        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                var script = event.script,
                    ...
                    isNestedRequest = this.state.nestedRequest && this.state.nestedRequest.isNestedRequest,
                ...
                this.host.on(EXECUTION_VAULT_BASE + executionId, async function (id, cmd, ...args) {
                    if (hasVaultAccess === undefined) {
                        try {
                            ...
                            hasVaultAccess = Boolean(await vaultSecrets?._?.allowScriptAccess(rootItemId));
                        }
                        catch (_) {
                            // eslint-disable-next-line require-atomic-updates
                    ...
                    if (vaultSecrets && vaultSecrets._) {
                        vaultSecrets._[`__hasVaultAccess_${rootItemId}`] = hasVaultAccess;
                    }
                ...
                const currentEventItem = event.parent && event.parent(),
                    ...
                    executeScript = ({ resolvedPackages }, done) => {
                        ...
                        this.host.execute(event, {
                            ...
                            if (vaultSecrets && vaultSecrets._ && vaultSecrets._[`__hasVaultAccess_${rootItemId}`]) {
