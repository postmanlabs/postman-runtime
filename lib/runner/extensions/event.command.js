    ...
    init: function (done) {
        ...
        sandbox.createContext(_.merge({
            ...
            context.on('execution.request', function (cursor, id, requestId, request) {
                Requester.create({
                    type: 'http',
                    source: 'script',  // @todo - get this from the sandbox
                    cursor: cursor
                }, run.options.requester, function (err, requester) {
                    if (err) {
                        // @todo is this correct? :-o
                        return run.triggers.exception.apply(run.triggers, arguments);
                    }
                    ...
                    return requester.request(sendId, new sdk.Request(request), function (err, res) {
                        context.dispatch('execution.response.' + id, requestId, err, res && (new sdk.Response(res)));
                    });
                });
            });
