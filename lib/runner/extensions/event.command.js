var _ = require('lodash'),
    ...
    hasVaultAccess = new Map(); // { [executionId: string]: Boolean }
...
const skippedExecutions = new Set(),
    ...
    parentExecutionCursors = new Map(),
    getParentRequestCursor = (executionId) => {
        return parentExecutionCursors.get(executionId);
    };
        ...
        sandbox.createContext(_.merge({
            ...
            const triggerHandler = function (eventName) {
                return function (...args) {
                    const executionId = args && args[0] && args[0].execution;

                    if (isExecutionSkipped(executionId)) { return; }

                    run.triggers[eventName](getParentRequestCursor(executionId) || args[0], ...args.slice(1));
                };
            };
    ...
    process: {
        ...
        event (payload, next) {
            var item = payload.item,
                ...
                requestExecutionStack = this.options.requestExecutionStack || [],
            ...
            async.mapSeries(events, function (event, next) {
                ...
                if (requestExecutionStack.length) {
                    // Add a reference to the parent execution's current cursor so any logs, exceptions etc are tagged
                    // to the pm.execution.runRequest line of parent request's scripts
                    const rootRequestCursor = requestExecutionStack[0].cursor;

                    parentExecutionCursors.set(executionId, rootRequestCursor);
                }
                ...
                this.host.on(EXECUTION_VAULT_BASE + executionId, async function (id, cmd, ...args) {
                    if (hasVaultAccess.get(executionId) === undefined) {
                        try {
                            // eslint-disable-next-line require-atomic-updates
                            hasVaultAccess.set(executionId, Boolean(await vaultSecrets?._?.allowScriptAccess(item.id)));
                        }
                        catch (_) {
                            // eslint-disable-next-line require-atomic-updates
                            hasVaultAccess.set(executionId, false);
                        }
                    }

                    if (requestExecutionStack.length) {
                        // For a nested request's vault access request
                        // mark the `hasVaultAccess` property as the consented value for
                        // parent executions as well if it hasn't already been set
                        // This is to make sure that mutations to vault secrets bubble up as expected
                        requestExecutionStack.forEach(function (parentReqExecution) {
                            if (hasVaultAccess.get(parentReqExecution.executionId) === undefined) {
                                hasVaultAccess.set(parentReqExecution.executionId, hasVaultAccess);
                            }
                        });
                    }
                    ...
                    if (!hasVaultAccess.get(executionId)) {
                ...
                this.host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                    runNestedRequest({ executionId, isExecutionSkipped, vaultSecrets }).bind(this));
                ...
                const currentEventItem = event.parent && event.parent(),
                    ...
                    executeScript = ({ resolvedPackages }, done) => {
                        ...
                        this.host.execute(event, {
                            ...
                            if (hasVaultAccess.get(executionId) && vaultSecrets) {
                                result.vaultSecrets = vaultSecrets;

                                // Prevent mutations from being carry-forwarded to subsequent events
                                if (!this.state.isNestedRequest) {
                                    // For nested requests, these mutations still have to bubble up to the parent
                                    // so keep accummulating these mutations between runs and disable tracking only
                                    // when the mutations from these secrets have reached the parent
                            ...
                            // Dispose any execution stack values no longer needed
                            parentExecutionCursors.delete(executionId);
                            hasVaultAccess.delete(executionId);
