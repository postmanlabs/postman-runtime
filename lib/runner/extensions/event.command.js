                ...
                this.host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                    function (scriptCursor, eventId, requestId, requestToRunId, nestedRequestOptions) {
                        ...
                        containerRequestPassedOptions.requestResolverBridge(requestToRunId,
                            async function (err, collectionWithOneReqItemToRun) {
                                if (err) {
                                    return dispatchErrorToListener(err);
                                }

                                if (!collectionWithOneReqItemToRun) {
                                    return dispatchErrorToListener(new Error('Expected a full normalized request item' +
                                        ' to invoke pm.execution.runRequest'));
                                }

                                // Prepare variables that have been set inside the pre-req/post-res script
                                // for this request so far. This is important because for nested requests,
                                ...
                                // till the script execution ends.
                                ...
                                const collectionRequestRunner = require('../../runner'),
                                    nestedReqLocalVariablesFromOptions = nestedRequestOptions.variables ?
                                    ...
                                    mergedLocalVariableValues = [
                                        ...
                                        ...(nestedReqLocalVariablesFromOptions)
                                    ],
                                    runnableCollection = new sdk.Collection({
                                        info: collectionWithOneReqItemToRun.info || {},
                                        item: [collectionWithOneReqItemToRun.item],
                                        variable: collectionWithOneReqItemToRun.variable ?
                                            collectionWithOneReqItemToRun.variable.map(function (variable) {
                                ...
                                new collectionRequestRunner().run(runnableCollection,
                                    ...
                                    function (err, run) {
                                        ...
                                        run.start({
                                            script (err, cursor, result, ...args) {
                                                // Sync mutations/changes to pm.variables, pm.environment & pm.globals
                                                ...
                                                if (result) {
                                                    ...
                                                    // Reset collection variable mutations from nested scripts
                                                    result.collectionVariables.mutations.stream = [];
                                                    result.collectionVariables.mutations.compacted = {};
                                                }

                                                // Propagate these changes to the parent script as well for usage in
                                                // further resolution + Persisting these mutations changes to a store
                                                if (self.triggers.script) {
                                                    self.triggers.script(err, cursor, result, ...args);
                                                }
                                            },
                                            exception (_, err) {
                                                if (err) {
