var _ = require('lodash'),
    ...
    // Rows served per `__pull` of a streamed pm.datasets result. Bounds memory
    // at every hop: the host holds one open engine cursor per stream and serves
    // a batch on demand; the sandbox pulls the next batch only as the script
    // iterates.
...
/**
 * Pull up to `limit` rows from an open pm.datasets stream iterator.
 * Resolves { rows, done }; done=true once the iterator is exhausted.
 *
 * @param {AsyncIterator} iterator - the host-side row iterator for one stream
 * @param {Number} limit - max rows to return in this batch
 * @returns {Promise<{rows: Array, done: Boolean}>}
 */
function pullDatasetBatch (iterator, limit) {
    var rows = [];

    function step () {
        return iterator.next().then(function (next) {
            if (next.done) { return { rows: rows, done: true }; }

            rows.push(next.value);

            return (rows.length >= limit) ? { rows: rows, done: false } : step();
        });
    }
    ...
    process: {
        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                // Open pm.datasets streams for this execution, keyed by streamId.
                // Each entry holds the host-side row iterator + a cancel fn; rows
                // are served one batch per `__pull` so no hop buffers the whole
                // result. (An entry lingers only if a script abandons a stream
                // without iterating to the end or breaking; it is released on the
                // next `__cancel`/exhaustion, else with the execution.)
                ...
                this.host.on(EXECUTION_DATASETS_BASE + executionId, function (id, cmd, datasetId, ...args) {
                    ...
                    // Streaming pull: serve the next row batch from an open stream.
                    // __pull/__cancel carry the streamId in their own arg (args[0]),
                    // not the datasetId slot (which is unused for stream control).
                    if (cmd === '__pull') {
                        const streamId = args[0],
                            entry = datasetStreams.get(streamId);

                        if (!entry) {
                            return dispatch('Datasets: unknown or closed stream');
                        }

                        return pullDatasetBatch(entry.iterator, DATASET_STREAM_BATCH).then((frame) => {
                            if (frame.done) { datasetStreams.delete(streamId); }
                    ...
                    // Streaming cancel: the script stopped iterating early.
                    if (cmd === '__cancel') {
                        const streamId = args[0],
                            entry = datasetStreams.get(streamId);
                    ...
                    try {
                        datasetsResolver(cmd, datasetId, args, function (err, result) {
                            ...
                            // sandbox then pulls rows via __pull. A materialised
                            // result (rows is a plain array) falls back to the
                            // legacy one-shot reply for back-compat.
                ...
                const currentEventItem = event.parent && event.parent(),
                    ...
                    executeScript = ({ resolvedPackages }, done) => {
                        ...
                        this.host.execute(event, {
                            ...
                            // the rows, or errored mid-stream. __pull-exhaustion/__cancel clears
                            // these cooperatively; this ensures a cursor never outlives its
                            // execution even when that handshake never completes.
                            datasetStreams.forEach(function (entry) { entry.cancel && entry.cancel(); });
