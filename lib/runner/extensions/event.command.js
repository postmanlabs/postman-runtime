        ...
        event: function (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.execute(event, {
                    ...
                    visualizerResults = result && result.return && result.return.visualizer;

                    if (visualizerResults) {
                        Visualizer.processTemplate(visualizerResults.template,
                            visualizerResults.data,
                            visualizerResults.options,
                            function (err, processedTemplate) {
                                if (err) {
                                    // bubble up the errors while processing template through visualizer results
                                    visualizerResults.error = err;

                                    return;
                                }

                                // add processed template and data to visualizer results
                                visualizerResults.processedTemplate = processedTemplate;
                                visualizerResults.globals = result.globals && result.globals.toObject();
                                visualizerResults.environment = result.environment && result.environment.toObject();

                                try {
                                    visualizerResults.response = result.response && result.response.json();
                                }
                                catch (err) {
