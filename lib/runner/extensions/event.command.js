var _ = require('lodash'),
    ...
    EXECUTION_DATASETS_STREAM_BASE = 'execution.datasets.stream.',
    DATASET_STREAM_PULL = 'pull',
    DATASET_STREAM_CANCEL = 'cancel',

    // Rows served per pull of a streamed pm.datasets result. Bounds memory at every
    // hop: the host holds one open engine cursor per stream and serves a batch on
    // demand; the sandbox pulls the next batch only as the script iterates.
    DATASET_STREAM_BATCH = 1000,
...
sanitizeFiles = function (request, callback) {
    ...
    var sanitisedFiles = [],
        bodyMode = request.body && _.toLower(request.body.mode);
    ...
    if (bodyMode === REQUEST_BODY_MODE_FILE) {
    ...
    process: {
        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                // Open pm.datasets streams for this execution, keyed by streamId. Each
                // entry holds the host-side row iterator plus the resolver's `cancel`,
                // and rows are served one batch per pull so no hop buffers the whole
                // result. Released on exhaustion, on cancel, on a mid-stream error, and
                // unconditionally when the execution tears down.
                let datasetStreams = new Map(),

                    // Release an open stream both ways: `cancel` frees a stream the script
                    // never began iterating (a not-yet-started generator's `finally` never
                    // runs), while `return()` resumes one that is mid-flight into its
                    // `finally`. Whichever is redundant is a no-op.
                    //
                    // Both are swallowed, sync and async: releasing is best-effort cleanup
                    // that every terminal runs, so a source that throws on the way out must
                    // not abort the teardown loop below (leaking the cursors after it), turn
                    // the last frame of a fully-delivered stream into a script error, or
                    // crash the process with an unhandled rejection. We still drop the
                    // cursor either way — a `cancel` that throws leaves us nothing better
                    // to do with it.
                    releaseDatasetStream = function (streamId, entry) {
                        datasetStreams.delete(streamId);

                        if (!entry) { return; }

                        util.safeCall(entry.cancel);

                        if (entry.iterator && typeof entry.iterator.return === 'function') {
                            // an async iterator's `return()` rejects asynchronously; the
                            // sync throw of a hand-rolled one is caught by safeCall
                            util.safeCall(function () {
                                const returned = entry.iterator.return();

                                returned && typeof returned.catch === 'function' &&
                                    returned.catch(function () { /* best-effort release */ });
                            });
                        }
                    };

                // Consumer contract: any runtime consumer (CLI, app, custom)
                // supplies `options.script.datasetsResolver` — a callback
                // `(cmd, datasetId, args, cb) => cb(err, { columns, rows, cancel })`.
                // When `rows` is an AsyncIterable the runner streams it to the
                // sandbox via the pull protocol below; a plain array works too.
                // A resolver wrapping dataset-engine-sdk returns exactly the shape
                // executeViewStream()/queryStream() produce.
                //
                // For the streaming form:
                //   - `cancel` is required, and must be idempotent — it can be called
                //     after the source has already released itself via its `finally`. A
                //     `cancel` that throws is swallowed rather than failing the run, so
                //     the cursor is dropped without the resolver learning about it.
                //   - a pull has no timeout here; a stalled source suspends the script in
                //     its `for await` until the sandbox's script timeout fires.
                //   - open streams are not capped. Calling executeView/executeQuery in a
                //     loop without iterating pins one cursor per call until teardown.
                this.host.on(EXECUTION_DATASETS_BASE + executionId, function (id, cmd, datasetId, ...args) {
                    ...
                    try {
                        datasetsResolver(cmd, datasetId, args, function (err, result) {
                            ...
                            // Streaming result: { columns, rows: AsyncIterable, cancel }.
                            // Register the open iterator and reply a head frame; the
                            // sandbox then pulls rows over the stream channel. A
                            // materialised result (rows is a plain array) falls back to
                            // the legacy one-shot reply for back-compat.
                            const rows = result && result.rows;

                            if (rows && typeof rows[Symbol.asyncIterator] === 'function') {
                                const streamId = `${executionId}:${id}`;

                                datasetStreams.set(streamId, {
                                    iterator: rows[Symbol.asyncIterator](),
                                    cancel: result.cancel,
                                    pulling: false
                                });

                                // Carry the rest of the resolver's result (e.g.
                                // `staleDatasources`) on the head frame — only `rows`
                                // and `cancel` are replaced by the stream.
                                return dispatch(null, _.assign(_.omit(result, ['rows', 'cancel']), {
                ...
                this.host.on(EXECUTION_DATASETS_STREAM_BASE + executionId, function (id, action, streamId) {
                    const entry = datasetStreams.get(streamId),

                        dispatch = (e, r) => {
                            this.host.dispatch(EXECUTION_DATASETS_STREAM_BASE + executionId, id, e, r);
                        };

                    // The script stopped iterating early. Idempotent: a stream the host
                    // has already closed is simply gone, which is not an error.
                    if (action === DATASET_STREAM_CANCEL) {
                        releaseDatasetStream(streamId, entry);

                        return dispatch(null, { ok: true });
                    }

                    if (action !== DATASET_STREAM_PULL) {
                        return dispatch(`Datasets: invalid stream action: ${action}`);
                    }

                    if (!entry) {
                        return dispatch('Datasets: unknown or closed stream');
                    }

                    // One pull at a time per stream: concurrent `next()` calls on the
                    // same iterator interleave rows and make an async generator throw.
                    if (entry.pulling) {
                        return dispatch('Datasets: stream already has a pull in flight');
                    }

                    entry.pulling = true;

                    return util.pullDatasetBatch(entry.iterator, DATASET_STREAM_BATCH).then((frame) => {
                        entry.pulling = false;

                        // Exhaustion is a terminal like any other: a hand-rolled
                        // async-iterable has no `finally` and would leak on this path.
                        frame.done && releaseDatasetStream(streamId, entry);

                        dispatch(null, frame);
                    }).catch((err) => {
                        entry.pulling = false;
                        releaseDatasetStream(streamId, entry);
                        dispatch(err instanceof Error ? err.message :
                ...
                const currentEventItem = event.parent && event.parent(),
                    ...
                    executeScript = ({ resolvedPackages }, done) => {
                        ...
                        this.host.execute(event, {
                            ...
                            this.host.removeAllListeners(EXECUTION_DATASETS_STREAM_BASE + executionId);

                            // Unconditional backstop: release any dataset cursors the script
                            // left open — it called executeView/executeQuery but never iterated
                            // the rows, or was disposed mid-stream. Exhaustion and cancel clear
                            // these cooperatively; this ensures a cursor never outlives its
                            // execution even when that handshake never completes.
                            datasetStreams.forEach(function (entry, streamId) {
                                releaseDatasetStream(streamId, entry);
                            });
                            datasetStreams.clear();
