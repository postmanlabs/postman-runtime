        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                (abortOnFailure || stopOnFailure) &&
                    this.host.on(EXECUTION_ASSERTION_EVENT_BASE + executionId, function (scriptCursor, assertions) {
                        _.forEach(assertions, function (assertion) {
                            // logToFs('assertion received', assertion);
                ...
                this.host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                    function (scriptCursor, id, requestId, requestToRunId, nestedRequestOptions) {
                        const self = this,
                        ...
                        collectionRequestResolver(requestToRunId, containerRequestPassedOptions.requestResolverBridge,
                            (err, resolvedRequestCollection) => {
                                if (err || !resolvedRequestCollection) {
                                    const error = serialisedError(err);

                                    delete error.stack;

                                    // Stop execution of the entire request
                                    return self.host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + id, requestId, error);
                                }
                                ...
                                new collectionRequestRunner().run(resolvedRequestCollection,
                                    ...
                                    function (err, run) {
                                        if (err) {
                                            return self.host
                                                .dispatch(EXECUTION_RESPONSE_EVENT_BASE + id, requestId, err);
                                        }
                                        ...
                                        run.start({
                                            ...
                                            response (err, _, response) {
                                                if (err) {
                                                    return self.host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + id,
                                                        requestId, err);
                                                }
