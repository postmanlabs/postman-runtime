        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                    function (scriptCursor, eventId, collectionRequestId, requestToRunId, nestedRequestOptions) {
                        const self = this,
                            containerRequestPassedOptions = self.state;

                        if (!containerRequestPassedOptions.requestResolverBridge) {
                            // TODO: This could be running in the postman cli, do we throw an error while this
                            // functionality isn't implemented or do we error out and stop execution.
                            // Because this will currently stop execution.
                            const error = serialisedError(new Error('pm.execution.runRequest(...) is not supported'));

                            delete error.stack;

                            return self.host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + eventId,
                                collectionRequestId, error);
                        }
                        ...
                        containerRequestPassedOptions.requestResolverBridge(requestToRunId,
                            function (err, normalizedRequestItemToExecute) {
                                if (
                                    err ||
                                    !normalizedRequestItemToExecute ||
                                    // Expect a full formed PostmanCollection.Item instance to account for scripts
                                    // responses, headers etc to be passed queued for an `item` execution
                                    !(normalizedRequestItemToExecute instanceof sdk.Item)
                                ) {
                                    const error = serialisedError(err);
                                ...
                                const localVariables = this.state._variables;

                                if (
                                    sdk.VariableScope.isVariableScope(localVariables) &&
                                    nestedRequestOptions.variables
                                ) {
                                ...
                                this.immediate('item', {
                                    ...
                                    source: 'script',
                                    abortOnError: true,
                                    isNestedRequest: true, // For use in postman-sandbox to stop destructive operations
                                    disableSNR: true, // Disables pm.execution.setNextRequest for scripts in the request
                                    data: getIterationData(this.state.data, scriptCursor.iteration),
                                    ...
                                    this.host.dispatch(EXECUTION_RESPONSE_EVENT_BASE + eventId,
                                        collectionRequestId,
                                        null,
                                        result && result.response,
                                        result && { cookies: result.cookies });
