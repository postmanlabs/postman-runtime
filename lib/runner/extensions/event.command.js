var _ = require('lodash'),
    ...
    hasVaultAccess = {}; // { [executionId: string]: Boolean }
        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_VAULT_BASE + executionId, async function (id, cmd, ...args) {
                    if (hasVaultAccess[executionId] === undefined) {
                        try {
                            // eslint-disable-next-line require-atomic-updates
                            hasVaultAccess[executionId] = Boolean(await vaultSecrets?._?.allowScriptAccess(item.id));
                        }
                        catch (_) {
                            // eslint-disable-next-line require-atomic-updates
                            hasVaultAccess[executionId] = false;
                        }
                    }

                    if (requestExecutionStack.length) {
                        // Nested request, mark the `hasVaultAccess` property as the inferred value for
                        // parent executions as well
                        requestExecutionStack.forEach(function (execution) {
                            hasVaultAccess[execution.executionId] = hasVaultAccess;
                        });
                    }
                ...
                const currentEventItem = event.parent && event.parent(),
                    ...
                    executeScript = ({ resolvedPackages }, done) => {
                        ...
                        this.host.execute(event, {
                            ...
                            if (hasVaultAccess[executionId] && vaultSecrets) {
