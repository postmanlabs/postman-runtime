            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_RUN_REQUEST_EVENT_BASE + executionId,
                    function (scriptCursor, eventId, requestId, requestToRunId, nestedRequestOptions) {
                        ...
                        if (containerRequestPassedOptions.requestNestingDepth > 5) {
                            return dispatchErrorToListener(new Error('Max depth for requests reached'));
                        }
                        ...
                        self.triggers.console(scriptCursor, 'log', 'executing nested referenced collection request');
                                ...
                                new collectionRequestRunner().run(runnableCollection,
                                    {
                                        ...
                                        timeout: nestedRequestOptions.timeout || containerRequestPassedOptions.timeout,
                                        localVariables: { values: mergedLocalVariableValues },
                                        // abortOnFailure: true,
                                        // requestNestingDepth: (self.state.requestNestingDepth || 0) + 1,
                                        host: {
                                            // Reuse current run's sandbox context to keep variables and other context
                                            // preserved across nested executions

                                            // TODO: Figure out context sharing
                                            // external: true,
                                            // instance: self.host,

                                            // For pm.exection.runRequest reqs, execution.setNextRequest is unavailable
                                            disabledAPIs: ['execution.setNextRequest']
                                        }
                                    },
                                    function (err, run) {
                                        if (err) {
                                        ...
                                        run.start({
                                            console (cursor, level, ...logs) {
                                                self.triggers.console(cursor, level, ...logs);
                                            },
                                            assertion (...args) {
                                                self.triggers.assertion(...args);
                                            },
                                            test (...args) {
                                                self.triggers.test(...args);
                                            },
                ...
                const currentEventItem = event.parent && event.parent(),
                    ...
                    executeScript = ({ resolvedPackages }, done) => {
                        ...
                        this.host.execute(event, {
                            ...
                            disabledAPIs: this.options.host && this.options.host.disabledAPIs ?
                                this.options.host.disabledAPIs : [],
