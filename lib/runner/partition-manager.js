...
class PartitionManager {
    ...
    runSinglePartition (index, localVariables, callback) {
        ...
        if (localVariables) {
            // customParallelIterations: the per-VU payload IS this VU's iteration-data
            // row (a plain object). Route it to `iterationData` so it surfaces as
            // pm.iterationData; the sandbox layers it into _variables too, so {{key}}
            // and pm.variables.get() keep resolving. _variables stays the pristine
            // per-VU script scope (owned solely by pm.variables.set()).
            if (this.runInstance.isCustomParallelIterations) {
