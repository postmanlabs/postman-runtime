...
module.exports = [
    ...
    function (context, run, done) {
        ...
        authHandler.post(authInterface, context.response, function (err, success) {
            ...
            var data = _.get(context, 'history.execution.data'),
                url = _.get(context, 'item.request.url');

            // replay with the url of the last request that was sent (take care of redirects)
            if (data && data.length > 1) {
                url.update(_.get(data[data.length - 1], 'request.href'));
            }
