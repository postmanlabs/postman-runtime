...
module.exports = {
    ...
    asyncEach: function (arr, fn, cb, ctx) {
        ...
        // if there are no functions, we simply move next
        if (!Array.isArray(arr)) { return cb.call(ctx, null, []); }
        ...
        var execute = function (err, value) {
            ...
            if (err || !arr.length) { return cb.call(ctx, err, results); }

            var item = arr.shift();

            try { fn.call(ctx, item, execute); }
            catch (e) { return cb.call(ctx, e, results); }
        };
