...
module.exports = {
    ...
    /**
     * Pull up to `limit` rows from an open async row iterator, to serve one batch of
     * a streamed `pm.datasets` result. `done` is true once the iterator is exhausted,
     * in which case `rows` holds whatever was read before it ran out (possibly fewer
     * than `limit`, possibly none).
     *
     ...
     */
    pullDatasetBatch (iterator, limit) {
        var rows = [];
