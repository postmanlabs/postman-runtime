...
module.exports = {
     ...
     *
     */
    createReadStream (resolver, fileData, callback) {
        // Return the stream directly if content is present
        if (fileData.content) {
            if (fileData.content instanceof Stream.Readable) {
                return callback(null, fileData.content);
            }

            var readStream;

            if (typeof fileData.content === 'string') {
                readStream = Buffer.from(fileData.content, 'base64');
            }
            else {
                readStream = Buffer.from(fileData.content);
            }
        ...
        if (!fileData.src || typeof fileData.src !== STRING) {
        ...
        return createReadStream(resolver, fileData.src, callback);
    }
};
