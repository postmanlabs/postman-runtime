...
module.exports = {
    ...
    createReadStream: function (resolver, fileSrc, callback) {
        ...
        resolver.stat(fileSrc, function(err, stats) {
            if (err) {
                // modify `ENOENT: no such file or directory` error message.
                err.code === 'ENOENT' && (err = new Error(`'${fileSrc}', no such file`));
                return callback(err);
            }
            ...
            // check for read permissions for anyof owner, group or other
            else if (!(stats.mode & 0o400 || stats.mode & 0o040 || stats.mode & 0o004)) {
                err = new Error(`'${fileSrc}', read permission denied`);
            }
            // create read stream if no error found so far
            else {
                try {
