...
function runNestedRequest ({ executionId, isExecutionSkipped, vaultSecrets, item }) {
    ...
    return function (cursor, eventId, requestId, requestToRunId, runRequestOptions = {}, runContext = {}) {
        const self = this,
            ...
            maxInvokableNestedRequests = _.get(self, 'options.requester.maxInvokableNestedRequests');

        self.state.nestedRequest = self.state.nestedRequest || {};
        ...
        self.state.nestedRequest.isNestedRequest = true;
        self.state.nestedRequest.rootCursor = self.state.nestedRequest.rootCursor || cursor;
        self.state.nestedRequest.rootItemId = self.state.nestedRequest.rootItemId || item.id;
        self.state.nestedRequest.invocationCount = self.state.nestedRequest.invocationCount || 0;

        self.state.nestedRequest.invocationCount += 1;
        ...
        requestResolver(requestToRunId, function (err, collection) {
            ...
            let clonedVaultSecrets,
                vaultAccessPropName = `__hasVaultAccess_${self.state.nestedRequest.rootItemId}`;
            ...
            new runner().run(runnableRefRequestCollection,
                {
                    ...
                    abortOnFailure: true,
                    nestedRequest: self.state.nestedRequest,
                    host: {
