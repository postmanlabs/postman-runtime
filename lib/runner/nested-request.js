...
function runNestedRequest ({ executionId, isExecutionSkipped, vaultSecrets, nRequestsInvoked }) {
    ...
    return function (cursor, eventId, requestId, requestToRunId, runRequestOptions = {}, runContext = {}) {
        const self = this,
            ...
            rootExecutionId = requestExecutionStack.length ? requestExecutionStack[0].executionId : executionId,
            requestsInvokedByRootExecution = Number(nRequestsInvoked.get(rootExecutionId)) || 0;
        ...
        if (requestsInvokedByRootExecution + 1 > maxInvokableNestedRequests) {
            return dispatchErrorToListener(new Error('Exceeded max number of requests per script' +
                ' invokable by pm.execution.runRequest'));
        }

        nRequestsInvoked.set(rootExecutionId, requestsInvokedByRootExecution + 1);

        // Check depth of current execution stack as well
