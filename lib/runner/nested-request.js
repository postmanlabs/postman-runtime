...
function runNestedRequest ({ executionId, isExecutionSkipped, vaultSecrets, item }) {
    ...
    return function (cursor, eventId, requestId, requestToRunId, runRequestOptions = {}, runContext = {}) {
        ...
        containerRequestState.nestedRequest = containerRequestState.nestedRequest || {};
        ...
        containerRequestState.nestedRequest.isNestedRequest = true;
        containerRequestState.nestedRequest.rootCursor = containerRequestState.nestedRequest.rootCursor || cursor;
        containerRequestState.nestedRequest.rootItemId = containerRequestState.nestedRequest.rootItemId || item.id;
        containerRequestState.nestedRequest.invocationCount = containerRequestState.nestedRequest.invocationCount || 0;

        containerRequestState.nestedRequest.invocationCount += 1;

        // No more than maxInvokableNestedRequests runRequest calls per script or any of its nested request scripts
        if (containerRequestState.nestedRequest.invocationCount > maxInvokableNestedRequests) {
        ...
        requestResolver(requestToRunId, function (err, collection) {
            ...
            let clonedVaultSecrets,
                vaultAccessPropName = `__hasVaultAccess_${containerRequestState.nestedRequest.rootItemId}`;
            ...
            new runner().run(runnableRefRequestCollection,
                {
                    ...
                    abortOnFailure: true,
                    nestedRequest: containerRequestState.nestedRequest,
                    host: {
                    ...
                    run.start({
                        script (_err, _cursor, result) {
                            ...
                            if (result) {
                                ...
                                if (clonedVaultSecrets &&
                                    clonedVaultSecrets._ &&
                                    clonedVaultSecrets._[vaultAccessPropName] !== undefined) {
                                    vaultSecrets._[vaultAccessPropName] = clonedVaultSecrets._[vaultAccessPropName];
                                }
                            }
                        },
                        exception (_, err) {
