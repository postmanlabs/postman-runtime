...
function runNestedRequest ({ executionId, isExecutionSkipped, vaultSecrets, item }) {
    ...
    return function (cursor, eventId, requestId, requestToRunId, runRequestOptions = {}, runContext = {}) {
        const self = this,
            ...
            nestedRequestsState = containerRequestState.nestedRequest || {};
        ...
        nestedRequestsState.isNestedRequest = true;
        nestedRequestsState.rootCursor = nestedRequestsState.rootCursor || cursor;
        nestedRequestsState.rootItemId = nestedRequestsState.rootItemId || item.id;
        nestedRequestsState.invocationCount = (nestedRequestsState.invocationCount || 0) + 1;

        // No more than maxInvokableNestedRequests runRequest calls per script or any of its nested request scripts
        if (nestedRequestsState.invocationCount > maxInvokableNestedRequests) {
        ...
        requestResolver(requestToRunId, function (err, collection) {
            ...
            let clonedVaultSecrets,
                vaultAccessPropName = `__hasVaultAccess_${nestedRequestsState.rootItemId}`;
            ...
            new runner().run(runnableRefRequestCollection,
                {
                    ...
                    abortOnFailure: true,
                    nestedRequest: nestedRequestsState,
                    host: {
