...
function runNestedRequest ({ executionId, isExecutionSkipped, vaultSecrets, item }) {
    ...
    return function (cursor, eventId, requestId, requestToRunId, runRequestOptions = {}, runContext = {}) {
        ...
        self.state.nestedRequest = _.defaults(self.state.nestedRequest || {}, {
            ...
            rootCursor: cursor,
            rootItem: item,
            nestingChain: [item.id]
        });

        self.state.nestedRequest.nestingChain = self.state.nestedRequest.nestingChain || [item.id];

        // eslint-disable-next-line one-var
        const nestingChain = self.state.nestedRequest.nestingChain,
            itemId = item.id,
            top = nestingChain[nestingChain.length - 1];

        if (top === itemId) {
            // Proceed, no nesting yet
        }
        else if (nestingChain.includes(itemId) && nestingChain.length > 1) {
            // We're back to an item's script after executing another nested item's script
            nestingChain.pop();
        }
        else {
            nestingChain.push(itemId);
        }

        // No more than maxInvokableNestedRequests nested depth
        if (nestingChain.length > maxInvokableNestedRequests) {
