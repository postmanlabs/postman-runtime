 ...
 *
 * @param {String} label -
 * @param {Number} [timestamp] - Optional cached timestamp to avoid syscall
 * @returns {Number}
 *
 * @example
 * var t = new Timings();
 ...
 */
Timings.prototype.record = function (label, timestamp) {
    // @note: Accept optional cached timestamp to reduce Date.now() syscalls
    // This is safe because timing precision within a single request/instruction is not critical
    return (this[label] = timestamp !== undefined ? timestamp : Date.now());
};
