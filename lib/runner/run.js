...
_.assign(Run.prototype, {
    ...
    // Release a streaming iteration-data source once the run ends by returning
    // its iterator (which runs the source's finally, e.g. cancel). No-op for the
    // array path, and idempotent (return() on a done iterator does nothing), so
    // it's safe to call at every terminal. `return()`'s promise is deliberately not
    // awaited, so a source whose `finally` rejects surfaces as an unhandled
    // rejection rather than a run error — it is expected to release without throwing.
    _closeIterationData () {
        var it = this.state && this.state._dataIterator;
    ...
    _process (callback) {
        ...
        if (!instruction) {
            ...
            // release a streaming iteration-data source (no-op for the array
            // path). Abort clears the pool, so it also lands here.
            this._closeIterationData();
        ...
        instruction.execute(function (err) {
            if (err) {
                this._closeIterationData(); // release on the error terminal too

                return callback(err, this.state.cursor.current());
            }

            return this._process(callback); // process recursively
        }, this);
    }
});
