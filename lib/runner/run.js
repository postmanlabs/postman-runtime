...
_.assign(Run.prototype, {
    ...
    // Release a streaming iteration-data source once the run ends. No-op for
    // the array path (and idempotent) so it's safe to call at every terminal.
    _closeIterationData () {
        var data = this.state && this.state.data;

        data && typeof data.close === 'function' && data.close();
    },
