...
_.assign(SshTunnelAgent.prototype, {
    ...
    createTunnel: function (sshConfig) {
        ...
        try {
            ...
            keyFile = fs.readFileSync(sshConfig.auth.privateKeyPath);
        }
        catch (e) {
            self.reporter.emit('tunnelError', e);
        }
        finally {
            sshc.connect({
                ...
                password: sshConfig.auth.password,
                passPhrase: sshConfig.auth.passPhrase,
                privateKey: keyFile
            });
        }
    ...
    forward: function (sshClient, sshConfig, sock) {
        ...
        sshClient.forwardOut(sock.localAddress, sock.localPort, sshConfig.auth.host,
            sshConfig.proxyPort, function(err, stream) {
                if (err) {
    ...
    start: function (proxyConfigList, localport, scope, cb) {
        ...
        self.localServer = net.createServer(function(sock) {
            ...
            if (pc) {
                ...
                error = new Error('No tunnel exists for the match ' + url);
        ...
        self.localServer.listen(localport, function (err) {
            (localport === 0) && (localport = self.localServer.address().port) &&
                self.setLocalProxy(scope, localport);
            ...
            self.reporter.on('ready', function(readyCount) {
                if (readyCount === 0) {
                    var err = new Error('No tunnels can be established');

                    return cb(err);
                }

                return cb(null, self.reporter);
            });
        });
    },
