...
pool = function (processors) {
    ...
    Instruction.prototype.execute = function(callback, scope) {
        ...
        args.push(this.payload, function (err) {
            // ensure callback is not called if the main function has faced an error
            if (errored) { return; }
            this.timings.record('end');
            this.out = _.clone(arguments);

            // in case it errored, we do not process any thenables
            // @todo - to be decided whether this remains the behaviour post .catch
            if (err) {
                return _.isFunction(callback) && callback.apply(scope, arguments);
            }
            ...
            _.isFunction(callback) && callback.apply(scope, arguments);
        }.bind(this));
        ...
        try {
            ...
            errored = true; // flag that it has errored
            this.out = [e];
            _.isFunction(callback) && callback.call(scope, e);
        }
    };
