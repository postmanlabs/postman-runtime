...
module.exports = [
    ...
    function (context, run, done) {
        ...
        // bail out if there is not matching authorizer for the type
        if (!authorizer) { return done(new Error('runtime: no authorizer')); }

        // bail out if authorizer has no pre hook
        if (!_.isFunction(authorizer.pre)) { return done(); }
         ...
         * 2. gives a no go,
         *      a: if there is `pre` responded with a request
         *          i. we suspend current request, send the intermediate request 
         *          ii: else we set response empty and
         *      b. invoke Auth `init` hook with the response of the intermediate request
         *      c: invoke Auth `pre` hook, and repeat from 1
         */
        function authPreHook() {
            authorizer.pre(auth, function (err, success, request) {
                // there was an error in pre hook of auth
                // @todo: should we drop the original request or go through without signing?
                if (err) { return done(err); }

                // authorizer gave a go, sign the request
                if (success) {
                    return authorizer.sign(
                        new sdk.RequestAuth(auth.toObjectResolved(
                            auth.parameters(),
                            [context.data, context.environment.syncVariablesTo({}), context.globals.syncVariablesTo({})]
                        )),
                ...
                if (!request) {
                    // invoke Auth `init` hook with empty response
                    return  authorizer.init(auth, null, function (error) {
                        // @todo: should we drop for errors in auth init
                        if (error) {
                            return done(error);
                        }
                ...
                // bailout if auth wanted an intermediate request, but did not implement `init` to catch the response
                if (!_.isFunction(authorizer.init)) {
                    return done(new Error(`runtime: ${auth.type} auth responded with an intermediate request,` +
                        'but has not implemented \'init\' to handle its response'));
                }
                ...
                // authorizer gave a no go, and an intermediate request.
                // make the intermediate request the response is passed to `init` hook
                replayController.requestReplay(context, item, true, function (err, response) {
                    // @todo: should we drop for errors in queued request
                    if (err) { return done(err); }

                    // pass the response to Auth `init` hook
                    authorizer.init(auth, response, function (error) {
                        // @todo: should we drop for errors in auth init
                        if (error) {
                            return done(error);
                        }
                    ...
                    // replay count exceeded, proceed with request
                    done();
                });
            });
        }
