        ...
        async.waterfall([async.constant(data), {
            ...
            formdata: function (formdata, next) {
                ...
                async.eachSeries(_.filter(formdata.all(), {type: 'file'}), function (formparam, callback) {
                    ...
                    var paramIsMultiSrc = Array.isArray(formparam.src);

                    // handle missing file src
                    if (!formparam.src || (paramIsMultiSrc && !formparam.src.length)) {
                        triggers.console(cursor, 'warn',
                            `Form param \`${formparam.key}\`, file load error: missing file source`);
                    ...
                    async.map(formparam.src, function (src, next) {
                        // eslint-disable-next-line security/detect-non-literal-fs-filename
                        util.createReadStream(resolver, src, function (err, stream) {
                            if (err) {
                                triggers.console(cursor, 'warn',
                                    `Form param \`${formparam.key}\`, file (${src}) load error: ${err.message || err}`);

                                // don't throw error or disable param if one of the src fails to load
                                return next();
                            }
                        ...
                        if (err) {
                            ...
                            run.triggers.console(cursor, 'warn',
                                `Form param \`${formparam.key}\` load error: ${err.message || err}`);
                            formparam.disabled = true;
                        ...
                        _.forEach(results, function (result) {
                            ...
                            result && formdata.insert(_.assign(formparam.toJSON(), result), formparam);
                        });
