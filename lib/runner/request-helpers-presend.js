const _ = require('lodash'),
    ...
    { oneNormalizedHeader } = require('../requester/core-header-utils'),
...
module.exports = [
    ...
    function (context, run, done) {
        ...
        if (!requestBody || requestBody.isEmpty()) {
            return done();
        }

        // if data mode is not raw bail out
        if (requestBody.mode !== RequestBody.MODES.raw) {
            return done();
        }

        // here we are using the core `getRequestBody` to cover the cases
        // where request body is not sent (BodyPruning is enabled)
        bodyParams = getRequestBody(request, context.protocolProfileBehavior);

        // bail out if there is not request body to sanitize
        if (!bodyParams) {
            return done();
        }

        // bail out if no comments present in raw body
        if (!(bodyParams.body.includes('//') || bodyParams.body.includes('/*'))) {
            return done();
        }

        contentTypeHeader = oneNormalizedHeader(request.headers, CONTENT_TYPE);

        if (contentTypeHeader &&
            typeof contentTypeHeader.value === STRING &&
            contentTypeHeader.value.startsWith(CONTENT_TYPE_LANGUAGE.json)) {
            requestBody.raw = stripJSONComments(bodyParams.body, { whitespace: false });
        }
