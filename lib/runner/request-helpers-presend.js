...
module.exports = [
    ...
    function (payload, run, done) {
        ...
        // Add cursor to the item, so that it can be used internally by the requester
        // todo - is this a good way? not sure :-|
        item._cursor = payload.coords;
    ...
    function (payload, run, done) {
        ...
        async.waterfall([
            // Pre verify
            function (cb) {
                run.authorizer.preVerify(item, run, cb);
            },
            // Call initialize if necessary
            function (verified, cb) {
                verified ? cb(null) : run.authorizer.init(item, run, function (err) {
                    return cb(err);
                });
            },
            // Sign the request
            function (cb) {
                run.authorizer.authorize(item, run, cb);
            }
        ], done);
    },
