        ...
        async.waterfall([async.constant(data), {
            ...
            formdata (formdata, next) {
                ...
                async.eachSeries(_.filter(formdata.all(), { type: 'file' }), function (formparam, callback) {
                    ...
                    if (formparam.content) {
                        util.createReadStream(resolver, formparam, function (err, stream) {
                            if (err) {
                                onLoadError(err, true);
                            }
                            else {
                                formparam.value = stream;
                                formparam.fileName = 'file';
                            }
                    ...
                    async.mapLimit(formparam, 10, function (formparam, next) {
                        // eslint-disable-next-line security/detect-non-literal-fs-filename
                        util.createReadStream(resolver, formparam, function (err, stream) {
                            if (err) {
            ...
            file (filedata, next) {
                // eslint-disable-next-line security/detect-non-literal-fs-filename
                util.createReadStream(resolver, filedata, function (err, stream) {
                    if (err) {
