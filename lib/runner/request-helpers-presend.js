    ...
    function (context, run, done) {
        ...
        var auth = context.auth,
            ...
            authSignHook = function () {
                try {
                    authHandler.sign(auth, context.item.request, function (err) { done(err); });
                }
                catch (e) { done(e); }
            };

        // bail out if there is not matching auth handler for the type
        if (!authHandler) { return done(new Error('runtime: no handler for auth')); }

        // sign and bail out if interactive mode is disabled
        if (!util.isInteractiveForAuth(run.options, authType)) { return authSignHook(); }
        ...
        authPreHook = function () {
            authHandler.pre(auth, function (err, success, request) {
                ...
                replayController.requestReplay(context, item, true, function (err, response) {
                    ...
                    authHandler.init(auth, response, function (error) {
