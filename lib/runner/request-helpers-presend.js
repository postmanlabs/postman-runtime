        ...
        async.waterfall([async.constant(data), {
            ...
            formdata: function (formdata, next) {
                ...
                async.eachSeries(_.filter(formdata.all(), {type: 'file'}), function (formparam, callback) {
                    ...
                    (function (cb) {
                        if (!Array.isArray(formparam.src)) {
                            // eslint-disable-next-line security/detect-non-literal-fs-filename
                            util.createReadStream(resolver, formparam.src, (err, stream) => {
                                if (!err) {
                        ...
                        async.map(formparam.src, function (src, done) {
                            // eslint-disable-next-line security/detect-non-literal-fs-filename
                            util.createReadStream(resolver, src, done);
                        }, function (err, streams) {
                            if (!err) {
                                _.forEach(streams, function (stream) {
                                    // Insert the items above the current formparam
                                    formdata.insert(_.assign({}, formparam, {value: stream}), formparam);
                                });

                                // remove the current formparam
