...
module.exports = [
    ...
    function (context, run, done) {
        ...
        async.waterfall([
            ...
            function (verified, next) {
                verified ? next(null) : run.authorizer.init(context, run, next);
            },
            // Sign the request
            function (next) {
    ...
    function (context, run, done) {
        ...
        async.waterfall([
            ...
            if (err) {
                run.triggers.console(payload.coords, 'warn', 'proxy lookup error: ' + (err.message || err));
            }
    ...
    function (context, run, done) {
        ...
        async.mapValues({
            ...
            if (err) {
                ...
                run.triggers.console(payload.coords, 'warn', 'certificate load error: ' + (err.message || err));
                return done();
            }
