        ...
        async.waterfall([async.constant(data), {
            ...
            formdata: function (formdata, next) {
                ...
                async.eachSeries(_.filter(formdata.all(), {type: 'file'}), function (formparam, callback) {
                    ...
                    // lack of stream means lack of `formparam.src` or missing file resolver method.
                    // @todo handle stream errors here instead of forwarding it to postman-request.
                    if (!stream) {
                        triggers.console(cursor, 'warn',
                            'unable to load form file for upload: "' + (formparam.src || formparam.key) + '"');
