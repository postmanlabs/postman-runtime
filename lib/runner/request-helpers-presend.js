    ...
    function (context, run, done) {
        ...
        function authPreHook() {
            authorizer.pre(auth, function (err, success, request) {
                ...
                if (success) {
                    return authorizer.sign(auth, context.item.request, function (err) {
                        done(err);
                    });
                }
                ...
                var replayController = new ReplayController(context.replayState, run),
                    tempRequest = sdk.Request.isRequest(request) ? request.toJSON() : new sdk.Request(request).toJSON(),
                    item = new sdk.Item({request: tempRequest});
