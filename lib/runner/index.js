    ...
    run: function (collection, options, callback) {
        ...
        extractRunnableItems(collection, options.entrypoint, function (err, runnableItems, entrypoint) {
            ...
            if (options.abortOnError && !entrypoint) {
                (_.isObject(options.entrypoint) && options.entrypoint.execute) ?
                    (errorPath = options.entrypoint.execute) :
                    (errorPath = options.entrypoint);
                return callback(new Error(`Unable to find a folder or request: ${errorPath}`));
            }
