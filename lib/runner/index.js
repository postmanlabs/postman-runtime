...
_.assign(Runner.prototype, {
    ...
    run: function (collection, options, callback) {
        ...
        runnableItems = Runner.extractRunnableItems(collection, options.entrypoint);
        ...
        if (options.abortOnError && !runnableItems.entrypoint) {
            return callback(new Error('A valid folder id/name is required'));
        }
...
_.assign(Runner, {
    ...
    extractRunnableItems: function (collection, groupId) {
        ...
        return {items: items, entrypoint: group};
    }
});
