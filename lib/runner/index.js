...
_.assign(Runner.prototype, {
    ...
    /**
     * A lazy, forward-only iteration-data source for bounded memory over large
     * data sets: an async-iterable of row objects (one per iteration) that also
     * exposes `length` (the total iteration count, known up front). The runner
     * pulls one row per iteration, never buffering the whole set, and calls the
     * iterator's `return()` on teardown to release the source. Nothing
     * dataset-specific — any consumer can build one, e.g. the dataset SDK's
     * `iterationSource(result, count)`. Serial-only; parallel runs must pass an
     * array instead.
     *
