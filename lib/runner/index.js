    ...
    run (collection, options, callback) {
        ...
        extractRunnableItems(collection, options.entrypoint, function (err, runnableItems, entrypoint) {
            if (err || !runnableItems) {
                // if it is one of the application defined errors, then return the same error
                // by default, the Error instance of node does not have a code set
                if (err.code && err.code.length > 0) {
                    return callback(err);
                }

                // else create a generic error
