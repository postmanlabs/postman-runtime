...
_.assign(Runner.prototype, {
     ...
     * @param {Object} [options.requestResolverBridge] - Resolver that receives an id from pm.execution.runRequest
     * and returns the JSON for the request collection. Should return an items array with the request and a 'variable'
     * array containing list of request-specific-collection variables.
     * @param {Boolean} [options.isNestedRequest] - Is this runner for a nested request? This is set by the parent
     * runner when spinning up a recursive runtime runner
     * @param {Number} [options.currentRequestNestingDepth] - The current depth at which nested requests are executing.
     * This is set by the parent runner when spinning up a recursive runtime runner
    ...
    run (collection, options, callback) {
        ...
        extractRunnableItems(collection, options.entrypoint, function (err, runnableItems, entrypoint) {
            ...
            return callback(null, (new Run({
                ...
                requestResolverBridge: options.requestResolverBridge,
                currentRequestNestingDepth: options.currentRequestNestingDepth,
                isNestedRequest: options.isNestedRequest,
