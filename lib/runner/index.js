var _ = require('lodash'),
    ...
    Run = require('./run'),
    IterationDataStream = require('./iteration-data-stream'),
...
_.assign(Runner.prototype, {
     ...
     * @param {Array.<Object>|Object} [options.data] - Iteration data. Either a
     *   materialised array (one object per iteration), OR a streaming source for
     *   bounded memory over large data sets. Any consumer (e.g. one using
     *   dataset-engine-sdk) can supply the streaming form:
     *     {
     *       __streamingIterationData: true,
     *       length: Number,             // total iteration count, known up front
     *       rows: AsyncIterable,        // yields one row object per iteration
     *       cancel: Function            // optional; releases the source early
     *     }
     *   e.g. { __streamingIterationData: true, length: await sdk.viewRowCount(d, v),
     *          rows: (await sdk.executeViewStream(d, v)).rows }. Rows are pulled
     *   through a sliding window (one per iteration), never fully buffered.
     *   Streaming is serial-only; parallel runs must pass an array.
    ...
    run (collection, options, callback) {
        ...
        extractRunnableItems(collection, options.entrypoint, function (err, runnableItems, entrypoint) {
            ...
            // Streaming iteration data: a lazy, bounded source instead of a
            // materialised array. We keep the runner's length-based machinery —
            // the row count is known up front — and feed rows through a sliding
            // window. Parallel runs need random cross-partition access, so they
            // are not supported here (callers buffer to an array for parallel).
            if (options.data && options.data.__streamingIterationData) {
                ...
                options.data = new IterationDataStream(options.data.rows, options.data.length, options.data.cancel);
            }
            else {
