...
_.assign(Runner.prototype, {
     ...
     *   bounded memory over large data sets: an async-iterable of row objects
     *   that also exposes a `length` (the total iteration count, known up front).
     *   Any consumer can supply it, e.g. the dataset SDK's `iterationSource(view,
     *   count)`. Rows are pulled one per iteration, never fully buffered; the
     *   runner calls the iterator's `return()` on teardown to release it.
