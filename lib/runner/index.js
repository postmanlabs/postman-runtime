...
_.extend(Runner.prototype, {
    ...
    run: function (collection, options, callback) {
        ...
        return callback(null, (new Run({
            items: Runner.extractRunnableItems(collection, options.entrypoint, options.requestMethod),
...
_.extend(Runner, {
     ...
     * @returns {Array}
     */
    extractRunnableItems: function (collection, groupId, method) {
        var items = [],
        ...
        group && group.forEachItem(function (item) {
            if(method && item.request.method != method){
                return;
            }
            items.push(item);
        });
