var _ = require('lodash'),
    ...
    setProxy = function(options, cb) {
        ...
        if (url && options.proxyList) {
            if(!_.isFunction(options.proxyList.isProxyConfigList)) {
              options.proxyList = new ProxyConfigList({}, options.proxyList);
            }
            _.isFunction(url.toString) && (url = url.toString());
            var proxyConfig = options.proxyList.resolve(url);
            if(proxyConfig) {
