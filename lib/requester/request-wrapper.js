var _ = require('lodash'),
    ...
    setProxy = function(request, options, cb) {
        ...
        if ((proxyConfig = _.get(request, 'proxy'))) {
            // todo - remove this when app does not need it anymore.
            _.set(request, '_._postman_proxy', proxyConfig);
    ...
    setCertificate = function (request, options, cb) {
        ...
        if (!isSSL || !(hasCertificate || hasDeprecatedCertificateManager)) {
            return cb(null, options);
        }

        // find certificate using deprecated certificateManager
        if (hasDeprecatedCertificateManager) {
            console.warn('Using certificateManager with requester options is deprecated.');
            ...
            options.certificateManager.getCertificateContents(request.url.getRemote(),
               function (err, info) {
                     ...
                     _.set(request, '_._postman_certificate', _.pick(info, ['pemPath', 'keyPath']));
                     cb(err, options);
                 });
        } else {
