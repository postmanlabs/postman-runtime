...
module.exports = {
    ...
    getRequestOptions: function (request, defaultOpts, protocolProfileBehavior) {
        ...
        tlsCipherSelection = protocolProfileBehavior.tlsCipherSelection || {};
        ...
        if (protocolProfileBehavior.preferServerCiphers) {
            options.honorCipherOrder = true;
        }

        // the SSL and TLS protocol versions to disabled during negotiation
        if (tlsCipherSelection.mode === 'disabledProtocols' && Array.isArray(tlsCipherSelection.disabledProtocols)) {
            tlsCipherSelection.disabledProtocols.forEach(function (protocol) {
        ...
        if (tlsCipherSelection.mode === 'customCiphers' && tlsCipherSelection.customCiphers) {
            options.ciphers = Array.isArray(tlsCipherSelection.customCiphers) ?
                tlsCipherSelection.customCiphers.join(':') : String(tlsCipherSelection.customCiphers);
        }
