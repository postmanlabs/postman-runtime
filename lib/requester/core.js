...
module.exports = {
     ...
     * (::ffff:a.b.c.d), and IPv4-compatible IPv6 (::a.b.c.d).
     *
    ...
    isAddressRestricted (host, networkOptions) {
        ...
        try {
            ...
            // handle IPv4-compatible IPv6 (::a.b.c.d) — unwrap that explicitly below
            var addr = ipaddr.process(strippedHost),
                parts;

            if (addr.kind() === 'ipv6') {
                parts = addr.parts;
                // IPv4-compatible form: first six groups are 0, seventh is non-zero
                // (guards against ::1 whose seventh group is also 0)
