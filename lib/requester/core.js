/**
 * @note
 * The architecture and interface in this file is utterly messed up due to our
 * evolution from XMLHttpRequest in Chrome App and then subsequent patch-work
 * done while adding features to runtime.
 *
 * @todo Move redundant behavior of Requester core to postman-request
 * (or the reverse)
 */
...
var dns = require('dns'),
    ...
    BodyTransformer = require('../request-body-transformer').BodyTransformer,
...
module.exports = {
     ...
     * @param callback
     */
    getRequestOptions: function (request, defaultOpts, protocolProfileBehavior, callback) {
        var options = {},
        ...
        self.getRequestBody(request, protocolProfileBehavior, function (bodyParams) {
     ...
     * @param callback
     */
    getRequestBody: function (request, protocolProfileBehavior, callback) {
        ...
        if (requestBodyType === BODY_MODE_TRANSFORM) {
            return BodyTransformer.parseBody(bodyContent, function (err, content) {
                if (err) {
                    return callback(); // @todo dont freak out, err will be fixed later
                }
        ...
        return callback(requestBodyBuilders[requestBodyType](bodyContent));
    },
