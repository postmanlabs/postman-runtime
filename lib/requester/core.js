...
var dns = require('dns'),
    ...
    lookup = function (lookupOptions, hostname, options, callback) {
        ...
        // Block *.localhost if any loopback address is restricted
        if (networkOpts.restrictedAddresses &&
            (self.isAddressRestricted(LOCALHOST, networkOpts) ||
                self.isAddressRestricted(LOCAL_IPV4, networkOpts) ||
                self.isAddressRestricted(LOCAL_IPV6, networkOpts))) {
            return callback(new Error(ERROR_ADDRESS_RESOLVE + hostname));
        }
...
module.exports = {
    ...
    getRequestOptions (request, defaultOpts, protocolProfileBehavior) {
        ...
        // intercept redirects to restricted addresses — Node's lookup hook is never
        // called for numeric hosts or bracketed IPv6 literals, so the DNS override
        // alone does not cover redirect paths
        if (networkOptions.restrictedAddresses) {
            redirectListener = function () {
                // postman-request re-processes options.bindOn on every redirect hop, so
                // multiple copies of this listener accumulate; once one has aborted the
                // request, bail out of the rest to avoid emitting `error` more than once
                if (this._aborted) { return; }

                var redirectHostname = this.uri && this.uri.hostname;

                if (self.isAddressRestricted(redirectHostname, networkOptions)) {
                    this.abort();
                    this.emit('error', new Error(ERROR_ADDRESS_RESOLVE + redirectHostname));
                }
            };

            // `options` is freshly built above, so bindOn is always unset here
            options.bindOn = { redirect: [redirectListener] };
        }
     ...
     * Supports both exact-match entries and CIDR range entries (e.g. '10.0.0.0/8').
     *
     * Handles bracketed IPv6 literals (e.g. '[::1]'), IPv4-mapped IPv6
     * (::ffff:a.b.c.d), IPv4-compatible IPv6 (::a.b.c.d), NAT64 (64:ff9b::/96,
     ...
     * they get the same IPv6-embedding normalization below — see the bug this fixes:
     * an exact-match entry like '169.254.169.254' used to be checked via string equality
     * only, so '::ffff:169.254.169.254'/'[::ffff:a9fe:a9fe]'/other equivalent encodings of
     * the exact same address bypassed it entirely), are parsed from restrictedAddresses on
     * first call and cached on the networkOptions object; restrictedAddresses is treated as
     * immutable after the first call — later mutations are not picked up.
     *
     * @param {String} host
     * @param {Object} networkOptions
     * @param {Object} networkOptions.restrictedAddresses
     *
     * @returns {Boolean}
     */
    isAddressRestricted (host, networkOptions) {
        ...
        // strip [ ] so IPv6 literals from URI hostnames are parseable by ipaddr.js
        hasBrackets = lowerHost[0] === '[' && lowerHost[lowerHost.length - 1] === ']';
        ...
        // This is a fast path for hostname entries (e.g. 'internal.corp') and the common
        // case; IP entries are also covered below regardless of this branch's outcome.
        ...
        // Every entry that parses as an IP — whether written as a CIDR ('10.0.0.0/8') or a
        // bare address ('169.254.169.254') — is included here, promoting bare addresses to
        // a /32 (IPv4) or /128 (IPv6) so they get matched against the *normalized* address
        // below instead of only ever being checked via string equality above. Entries that
        // fail to parse as either (e.g. a hostname like 'internal.corp') are skipped here —
        // they remain covered by the exact-match branch above only.
                ...
                .reduce(function (acc, key) {
                    ...
                    try {
                        if (_.includes(key, '/')) {
                            ...
                            parsed = ipaddr.parse(key);
                            acc.push([parsed, parsed.kind() === 'ipv6' ? 128 : 32]);
                        }
                    }
                    // silently ignore entries that are neither a valid CIDR nor a valid IP
                    // (e.g. a hostname entry); the entry simply never matches here rather
                    // than throwing on every lookup
                    catch (e) { /* not a parseable IP/CIDR, skip it */ }

                    return acc;
                }, []);
        }

        if (!networkOptions.restrictedCidrs.length) { return false; }

        // IPv4-compatible IPv6 (::a.b.c.d) is deprecated and ipaddr.js refuses to parse
        // it, so detect the shape and match on the embedded IPv4 directly. Colon-only
        // forms with no dotted quad (e.g. `::1`) and `::ffff:a.b.c.d` fail the checks
        // below and are handled by ipaddr.process(). The embedded IPv4 is matched as-is,
        // including 0.x.x.x forms, so a 0.0.0.0/8 restriction can't be bypassed via
        // `::0.x.x.x`.
        if (_.startsWith(strippedHost, '::') && _.includes(strippedHost, '.') &&
            ipaddr.IPv4.isValid(strippedHost.slice(2))) {
            compatEmbedded = strippedHost.slice(2);
        }

        try {
            if (compatEmbedded) {
                addr = ipaddr.parse(compatEmbedded);
            }
            else {
                // process() unwraps IPv4-mapped IPv6 (::ffff:a.b.c.d) to IPv4; does NOT
                // handle the other IPv4-embedding encodings — unwrap those explicitly below
                addr = ipaddr.process(strippedHost);
                ...
                if (addr.kind() === 'ipv6') {
                    ...
                    if (range === 'rfc6052' || range === 'rfc6145') {
                        ...
                        // both embed the IPv4 address in the last 4 bytes
            ...
            return networkOptions.restrictedCidrs.some(function (cidr) {
                try { return addr.match(cidr); }
                catch (e) { return false; } // family mismatch — skip to next CIDR
            });
        }
        catch (e) { /* host is not a parseable IP address */ }
