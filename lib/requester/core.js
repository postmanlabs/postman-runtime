var _ = require('lodash'),
    ...
    METHODS_WITHOUT_BODY = ['get', 'copy', 'head', 'purge', 'unlock'],
...
module.exports = {
    ...
    getRequestBody: function (request, options) {
        ...
        var requestBody = request.body,
            ...
            forceInclude = requestBody.forceInclude;

        // early bail out for empty body (this area has some legacy shenanigans)
        if (bodyIsEmpty) {
            return;
        }

        // bail out if request method doesn't support body and forceInclude is not set.
        // @note `!_.isNil` check is required to avoid bail out for undefined case
        //       and respect sendBodyWithGetRequests option in next check.
        // @todo cleanup this when sendBodyWithGetRequests support is dropped.
        if (_.includes(METHODS_WITHOUT_BODY, requestMethod) && !_.isNil(forceInclude) && !forceInclude) {
            return;
        }

        // bail out if sendBodyWithGetRequests is set.
        // @note request level option (forceInclude) has higher precedence than requester option.
        // @todo deprecate sendBodyWithGetRequests option
        if (requestMethod === GET && !forceInclude && !options.sendBodyWithGetRequests) {
            return;
        }
