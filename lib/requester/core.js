...
var dns = require('dns'),
    ...
    _lookup = function (options, hostLookup, hostname, callback) {
        ...
        if (resolvedAddr) {
            ...
            return setImmediate(function () {
                // When options.all is set, the callback expects array of addresses instead of a single address
                return options.all ? callback(null, [{ address: resolvedAddr, family: resolvedFamily }]) :
    ...
    lookup = function (lookupOptions, hostname, options, callback) {
        ...
        if (getTLD(lowercaseHost) !== LOCALHOST) {
            ...
            if (options.all) {
                return _lookup(options, hostLookup, lowercaseHost, function (err, addresses) {
                    ...
                    addresses = _.filter(addresses, (addr) => {
                        return !self.isAddressRestricted(addr.address, networkOpts);
                    });

                    return callback(_.isEmpty(addresses) ?
                        new Error(ERROR_ADDRESS_RESOLVE + hostname) : null, addresses);
                });
            }
        ...
        connect(LOCAL_IPV6, lookupOptions.port, function (err) {
            ...
            if (err) {
                // When options.all is set, the callback expects array of addresses instead of a single address
                return options.all ? callback(null, [{ address: LOCAL_IPV4, family: 4 }]) :
                    callback(null, LOCAL_IPV4, 4);
            }

            // When options.all is set, the callback expects array of addresses instead of a single address
            return options.all ? callback(null, [{ address: LOCAL_IPV6, family: 6 }]) :
