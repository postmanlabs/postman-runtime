...
module.exports = {
    ...
    getRequestOptions (request, defaultOpts, protocolProfileBehavior) {
        ...
        if (networkOptions.restrictedAddresses) {
            ...
            if (options.bindOn && options.bindOn.redirect) {
                options.bindOn.redirect.push(redirectListener);
            }
            else if (options.bindOn) {
    ...
    isAddressRestricted (host, networkOptions) {
        ...
        if (!restrictedAddresses) { return false; }
        ...
        if (restrictedAddresses[lowerHost] || restrictedAddresses[strippedHost] ||
            (!hasBrackets && lowerHost.indexOf(COLON) !== -1 &&
        ...
        if (!networkOptions.restrictedCidrs) {
            networkOptions.restrictedCidrs = Object.keys(restrictedAddresses)
                .filter(function (key) {
                    return restrictedAddresses[key] && key.indexOf('/') !== -1;
                })
                .reduce(function (acc, key) {
                    try { acc.push(ipaddr.parseCIDR(key)); }
                    catch (e) {
                        console.warn('postman-runtime: invalid CIDR in restrictedAddresses, entry ignored: ' + key);
                    }
        ...
        try {
            ...
            var addr = ipaddr.process(strippedHost),
                range, parts;
            ...
            if (addr.kind() === 'ipv6') {
                ...
                // IPv4-compatible form (::x.x.x.x written as 0:0:0:0:0:0:x:x) — range() returns
                // 'unicast' so range-based detection can't be used; fall back to parts check.
                // Guards against ::1 whose parts[6] is also 0.
