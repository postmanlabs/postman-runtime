...
module.exports = {
    ...
    getRequestBody: function (request, options) {
        ...
        var requestBody = request.body,
            requestBodyType = _.get(request, 'body.mode'),
            requestMethod = _.isString(request.method) ? request.method.toLowerCase() : undefined,
            bodyIsEmpty = requestBody.isEmpty(),
            bodyContent = requestBody[requestBodyType];

        // early bailout for empty body (this area has some legacy shenanigans)
        // @todo support a body type where nothing is sent and remove this GET restriction
        if (bodyIsEmpty || (requestMethod === GET && !options.sendBodyWithGetRequests)) {
            return;
        }
        ...
        //       the builder module
        if (!_.has(requestBodyBuilders, requestBodyType)) {
            return;
        }
