...
module.exports = {
    ...
    getRequestOptions: function (request, defaultOpts, protocolProfileBehavior) {
        ...
        // override DNS lookup
        if (
            networkOptions.restrictedAddresses ||
            hostname === LOCALHOST ||
            (!isTunnelingProxy && proxyHostname === LOCALHOST) ||
            networkOptions.hostLookup
        ) {
            portNumber = Number(port) || (isSSL ? HTTPS_DEFAULT_PORT : HTTP_DEFAULT_PORT);

            _.isFinite(portNumber) && (options.lookup = lookup.bind(this, {
                port: (request.proxy && !isTunnelingProxy) ? Number(request.proxy.port) : portNumber,
