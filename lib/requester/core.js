var _ = require('lodash'),
    ...
    PPB_OPTS = {
        ...
        // Controls whether or not request should be sent with cache-control header, if true request will contain
        // 'cache-control: no-cache' header
        noCacheHeader: 'noCacheHeader',

        // Controls whether or not request should be sent with postman-token header
        postmanTokenHeader: 'postmanTokenHeader'
    },
...
module.exports = {
     ...
     * @param defaultOpts.noCacheHeader
     * @param defaultOpts.postmanTokenHeader
    ...
    getRequestOptions: function (request, defaultOpts, protocolProfileBehavior) {
        ...
        (_.isUndefined(options.noCacheHeader) ? true : options.noCacheHeader) &&
            self.ensureHeaderExists(options.headers, 'Cache-Control', 'no-cache');

        (_.isUndefined(options.postmanTokenHeader) ? true : options.postmanTokenHeader) &&
            self.ensureHeaderExists(options.headers, 'Postman-Token', uuid.v4());
