...
module.exports = {
    ...
    getRequestOptions (request, defaultOpts, protocolProfileBehavior) {
        ...
        // intercept redirects to raw IP literals — Node's lookup hook is never
        // called for numeric hosts, so the DNS override alone does not cover this path
        if (networkOptions.restrictedAddresses) {
            options.bindOn = {
                redirect: [function () {
                    var redirectHostname = this.uri && this.uri.hostname;

                    if (net.isIP(redirectHostname) &&
                        self.isAddressRestricted(redirectHostname, networkOptions)) {
                        this.abort();
     ...
     * @param {Array} networkOptions.restrictedCidrs - pre-parsed CIDR tuples from RequesterPool
     *
     * @returns {Boolean}
     */
    isAddressRestricted (host, networkOptions) {
        ...
        // exact match — existing behavior, unchanged
        if (networkOptions.restrictedAddresses && networkOptions.restrictedAddresses[lowerHost]) {
            return true;
        }

        // CIDR range check against pre-parsed entries
        if (networkOptions.restrictedCidrs && networkOptions.restrictedCidrs.length) {
            try {
                // process() unwraps IPv4-mapped IPv6 (::ffff:a.b.c.d) to IPv4 so it
                // matches IPv4 CIDRs correctly; plain IPv4/IPv6 are unaffected
                var addr = ipaddr.process(lowerHost);
