var dns = require('dns'),
    ...
    ALLOWED_DISABLED_SYSTEM_HEADERS = {
        'content-length': true,
        'content-type': true,
        'user-agent': true,
...
module.exports = {
    ...
    getRequestOptions: function (request, defaultOpts, protocolProfileBehavior) {
        ...
        disabledSystemHeaders.host && (options.setHost = false);

        disabledSystemHeaders.connection && !request.headers.has('connection') &&
            (options.noConnectionHeader = true);

        options.transformRequest = function (req) {
            _.forEach(ALLOWED_DISABLED_SYSTEM_HEADERS, function (value, key) {
                if (!disabledSystemHeaders[key]) { return; }

                header = request.headers.one(key);

                if (!header || header.system || header.disabled) {
