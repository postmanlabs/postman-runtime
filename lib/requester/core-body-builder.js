...
var _ = require('lodash'),
    ...
    GRAPHQL_PROPS = ['query', 'operationName', 'variables'],
    GRAPHQL_VARIABLES_PROP = 'variables',
...
module.exports = {
    ...
    graphql: function (content, request) {
        ...
        // @note this assumes that string content is escaped correctly
        body = [];

        GRAPHQL_PROPS.forEach(function (prop) {
            if (!(content.hasOwnProperty(prop) && typeof content[prop] === STRING)) {
                return;
            }

            // `variables` is already a JSON string, add as it is
            if (prop === GRAPHQL_VARIABLES_PROP) {
                return body.push('"' + prop + '":' + content[prop]);
            }

            body.push('"' + prop + '":"' + content[prop] + '"');
        });

        return {
            body: body.length ? '{' + body.join(',') + '}' : E
        };
    }
};
