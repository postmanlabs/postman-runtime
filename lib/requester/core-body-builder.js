...
var _ = require('lodash'),
    ...
    /**
     * Use to map content-type from body options
     *
     * @private
     * @type {Object}
     */
...
module.exports = {
    ...
    raw: function (content, request) {
        var contentLanguage = _.get(request, 'body.options.raw.language'),
            contentTypeHeader;


        // Add `Content-Type` header from body options if not set already
        if (contentLanguage && request && !request.headers.has(CONTENT_TYPE_HEADER.key)) {
            contentTypeHeader = CONTENT_TYPE[contentLanguage];
            request.headers.add({
    ...
    graphql: function (content, request) {
        ...
        if (request && !request.headers.has(CONTENT_TYPE_HEADER.key)) {
            request.headers.add({
                key: CONTENT_TYPE_HEADER.key,
