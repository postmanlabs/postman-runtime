...
module.exports = {
    ...
    raw: function (content, request) {
        var mode = _.get(request, 'body.mode'),
            options = _.get(request, 'body.options'),
            contentType,
            contentLanguage;

        // Add `Content-Type` header from body options if not set already
        if (options && !request.headers.has(CONTENT_TYPE_HEADER.key)) {
            contentType = _.get(options[mode], 'contentType');
            if (contentType) {
                contentLanguage = LANGUAGE[contentType];
                request.headers.add({
