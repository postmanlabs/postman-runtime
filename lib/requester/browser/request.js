...
function request(options, callback) {
    ...
    var multipart = function(obj) {
        ...
        var hasFile = false;
        for (var p in obj) {
            if (obj.hasOwnProperty(p) && obj[p].toString && (obj[p].toString() === '[object File]')) {
                hasFile = true;
                break;
            }
        }

        // If we have files, use the Browser API
        if (hasFile) {
        ...
        // If no files are attached, construct the raw body ourselves, so that we can control the boundary etc.
        var result = {};
