 ...
 * @param {Function} callback -
 */
function sanitizeRawBody (request, disabledSystemHeaders, callback) {
    ...
    // bail out if data mode is not not raw
    if (request.body.mode !== BODY_MODE.raw) {
        return callback(null, request);
    }

    contentTypeHeader = oneNormalizedHeader(request.headers, CONTENT_TYPE, disabledSystemHeaders);
    ...
    if (contentTypeHeader &&
        ...
        request.body.raw = stripJSONComments(request.body.raw, { whitespace: false });
    }
...
function dryRun (request, options, done) {
    ...
    async.waterfall([
        ...
        function sanitizeRawBodyContent (request, next) {
            sanitizeRawBody(request, disabledSystemHeaders, next);
        },
