...
RequesterPool = function (options, callback) {
    ...
    // pre-parse any CIDR entries in restrictedAddresses so isAddressRestricted
    // doesn't re-parse on every DNS lookup callback
    (function () {
        var networkOpts = self.options.network,
            restrictedAddresses = networkOpts && networkOpts.restrictedAddresses;

        if (!restrictedAddresses) { return; }

        networkOpts.restrictedCidrs = Object.keys(restrictedAddresses)
            .filter(function (key) {
