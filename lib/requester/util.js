var _ = require('lodash'),
     ...
     *
     * @type {{RAW: string, URLENCODED: string, FORMDATA: string, FILE: string}}
     */
    REQUEST_MODES = {
        RAW: 'raw',
...
module.exports = {
    ...
    getRequestBody: function (request, options) {
        var mode = _.get(request, 'body.mode'),
            ...
            empty = body && body.isEmpty && body.isEmpty(),
            method,
            content,
            computedBody;

        if (!request || empty || !mode || !body[mode]) {
            return;
        }

        // Do not send a body if the request is GET and sendBodyWithGetRequests is falsey.
        method = _.isString(request.method) ? request.method.toLowerCase() : undefined;
        if (method === GET && !options.sendBodyWithGetRequests) {
            return;
        }
