var _ = require('lodash'),
     ...
     * @param lookupOptions.restrictedAddresses
     * @param {Object.<String, String | {ip: String, family: Number}>} lookupOptions.hostIpMap An object containing host
     * to ip mapping. The value is either a string (family defaults to 4 in this case) or an object having ip and family
     * @param hostname
     * @param options
     * @param callback
     */
    lookup = function (lookupOptions, hostname, options, callback) {
        ...
        if (lowercaseHost !== LOCALHOST) {
            if (lookupOptions.hostIpMap && (resolvedAddr = lookupOptions.hostIpMap[lowercaseHost])) {
                typeof resolvedAddr === 'string' && (resolvedAddr = {ip: resolvedAddr, family: 4});
                return callback(null, resolvedAddr.ip, resolvedAddr.family || 4);
            }

            return dns.lookup(hostname, options, function (err, address, family) {
                if (err) { return callback(err); }
                callback(self.isAddressRestricted(address, lookupOptions) ?
                    new Error(ERROR_ADDRESS_RESOLVE + hostname) : err, address, family);
            });
        }
...
module.exports = {
    ...
    getRequestOptions: function (request, defaultOpts) {
        ...
        // override DNS lookup
        if (defaultOpts.restrictedAddresses || hostname.toLowerCase() === LOCALHOST || defaultOpts.hostIpMap) {
            ...
            _.isFinite(portNumber) && (options.lookup = lookup.bind(this, {
                port: portNumber,
                hostIpMap: defaultOpts.hostIpMap,
                restrictedAddresses: defaultOpts.restrictedAddresses
            }));
        }
     ...
     * @param {Array<String>} options.restrictedAddresses
     *
     * @returns {Boolean}
     */
    isAddressRestricted: function (host, options) {
        return options.restrictedAddresses && options.restrictedAddresses[(host && host.toLocaleLowerCase())];
    }
};
