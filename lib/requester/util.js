var _ = require('lodash'),
     ...
     * @param lookupOptions.network.hosts
     * @param lookupOptions.network.hosts.type
     * @param hostname
     * @param options
     * @param callback
     */
    lookup = function (lookupOptions, hostname, options, callback) {
        var self = this,
            ...
            hostsOptions = networkOpts.hosts,
            hostIpMap,
            resolvedFamily = 4,
            resolvedAddr;

        hostsOptions && hostsOptions.type === HOSTS_TYPE.HOST_IP_MAP &&
            (hostIpMap = hostsOptions[HOSTS_TYPE.HOST_IP_MAP]) && (resolvedAddr = hostIpMap[lowercaseHost]);

        if (resolvedAddr) {
            _.includes(resolvedAddr, COLON) && (resolvedFamily = 6);

            // returning error synchronously causes uncaught error
            return setImmediate(function () {
                callback(self.isAddressRestricted(resolvedAddr, networkOpts) ?
                    new Error(ERROR_ADDRESS_RESOLVE + hostname) : null, resolvedAddr, resolvedFamily);
            });
        }
        ...
        if (lowercaseHost !== LOCALHOST) {
            return dns.lookup(hostname, options, function (err, address, family) {
                ...
                callback(self.isAddressRestricted(address, networkOpts) ?
...
module.exports = {
    ...
    getRequestOptions: function (request, defaultOpts) {
        ...
        // override DNS lookup
        if (!_.isEmpty(networkOptions) || hostname.toLowerCase() === LOCALHOST) {
    ...
    isAddressRestricted: function (host, networkOptions) {
        return networkOptions.restrictedAddresses &&
            networkOptions.restrictedAddresses[(host && host.toLocaleLowerCase())];
    }
};
