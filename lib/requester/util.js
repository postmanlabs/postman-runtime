var _ = require('lodash'),
     ...
     * @param callback
     */
    _lookup = function (options, networkOpts, hostname, callback) {
        var hostsOptions = networkOpts.hosts,
            hostIpMap,
            resolvedFamily = 4,
            resolvedAddr;

        hostsOptions && hostsOptions.type === HOSTS_TYPE.HOST_IP_MAP &&
            (hostIpMap = hostsOptions[HOSTS_TYPE.HOST_IP_MAP]) && (resolvedAddr = hostIpMap[hostname]);
     ...
     * @param lookupOptions.network.hosts.hostIpMap
     * @param hostname
     * @param options
     * @param callback
     */
    lookup = function (lookupOptions, hostname, options, callback) {
        ...
        lowercaseHost !== LOCALHOST && _lookup(options, networkOpts, lowercaseHost, function (err, addr, family) {
...
module.exports = {
    ...
    getRequestOptions: function (request, defaultOpts) {
        ...
        // override DNS lookup
        if (networkOptions.restrictedAddresses || hostname.toLowerCase() === LOCALHOST || networkOptions.hosts) {
