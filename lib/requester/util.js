            ...
            options = {
                formData: _.reduce(content, function (accumulator, param) {
                    var isFile = (param.type === 'file'),
                        fileResolver = defaults.fileResolver;

                    if (isFile && !fileResolver) { // It's a file, but we don't have a resolver for it
                        return accumulator;
                    }

                    if (isFile && param.src) {
                        param.value = fileResolver ? fileResolver.createReadStream(param.src) : '';
                    }
