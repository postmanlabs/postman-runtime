var _ = require('lodash'),
    ...
    /**
     * A helper function similar to Node's `util.inherits`
     *
     * @param child
     * @param parent
     */
 ...
 * @constructor
 */
inherits(Requester = function (source, options) {
    this.options = {
 ...
 */
Requester.prototype.request = function (item, sendId, cursor, callback) {
    var self = this,
    ...
    return requests(request, requestOptions, function (err, res, resBody) {
        if (err) {
            ...
            self.emit(sendId, err, self.source, undefined, request);
        ...
        self.emit(sendId, null, self.source, response, request, cookies);

        callback(null, response, request, cookies);
    });
};

Requester.create = function (source, options, callback) {
    return callback(null, new Requester(source, options));
};
