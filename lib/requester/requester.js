var requests = require('./request-wrapper'),
    ...
    /**
     * This is a default trace object returned in all requester IO events.
     *
     * It is mostly here for future-proofing the IO event API, because with requests being replayed, sent from auth
     * helpers etc, we will need to maintain a trace history.
     *
     * The structure captures the essence of what we need to store.
     *
     * @todo Remove this when the authorizer comes in and we have proper tracing implemented.
     */
    DEFAULT_TRACE = {
        history: [],
        current: {
...
Requester.prototype.request = function (item, triggers, cursor, callback) {
    ...
    return requests(request, requestOptions, function (err, res, resBody) {
        if (err) {
            ...
            triggers.io(err, cursor, DEFAULT_TRACE, undefined, request);
        ...
        triggers.io(null, cursor, DEFAULT_TRACE, response, request);
