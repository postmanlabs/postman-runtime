    ...
    request: function (id, request, protocolProfileBehavior, callback) {
        ...
        return requests(request, requestOptions, function (err, res, resBody) {
            ...
            _.forOwn(responseJSON.request && responseJSON.request.headers, function (value, key) {
                ...
                // cookies get appended with `;` so handle them separately
                else if (header && key.toLowerCase() === COOKIE_HEADER_KEY) {
                    header.value = value;
                }
                // referer header gets updated on redirects
                else if (header && key.toLowerCase() === REFERER_HEADER_KEY) {
                    header.value = value;
                }
            });
