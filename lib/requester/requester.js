...
_.assign(Requester.prototype, /** @lends Requester.prototype */ {
    ...
    request: function (id, request, protocolProfileBehavior, callback) {
        ...
        return requests(request, requestOptions, function (err, res, resBody) {
            ...
            rawRequestHeader = _.has(res, 'request.req._header') ? res.request.req._header : '';

            // request body content length
            requestContentLength = (typeof (res.caseless && res.caseless.get) === FUNCTION) ?
                Number(res.caseless.get('content-length')) : 0;
