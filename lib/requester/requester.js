...
inherits(Requester = function (trace, options) {
    this.options = {
        ...
        // @todo: Short circuiting the options like this on the fly is bad.
        // @todo: To fix this, the Collection SDK needs to expose a ToughCookie compatible jar that can be passed into
        // @todo: the run options. This ensures that the dependency from postman-request can be removed.
        // eslint-disable-next-line max-len
        cookieJar: options && options.cookieJar || (options ? (options.cookieJar = RequestCookieJar()) : (options = { cookieJar: RequestCookieJar() })),
