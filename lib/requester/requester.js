...
_.assign(Requester.prototype, /** @lends Requester.prototype */ {
    ...
    request: function (id, request, protocolProfileBehavior, callback) {
        var self = this,
            ...
            isCollectionRequest = self.trace.source === 'collection',
            startTime = Date.now(),
            ...
            onStart = function (res) {
                ...
                // bail out if its a pm.sendRequest or auth request
                if (!isCollectionRequest) { return; }

                var responseStartEvent = RESPONSE_START_EVENT_BASE + id,
                    responseJSON = core.jsonifyResponse(res, requestOptions),
                ...
                self.emit(responseStartEvent, null, response, request, cookies);
            };
        ...
        return requests(request, requestOptions, onStart, function (err, res, resBody, debug) {
            ...
            complete(null, response, history);
        });
    },
