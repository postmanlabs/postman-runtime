...
var _ = require('lodash'),
    ...
    /**
     * Method to determine if the top level domain associated with a request
     * is special use domain.
     * 
     * @param {string} domain - The cannonical domain which is associated with the request
     * @returns {boolean}
     */
    isSpecialDomain = function (domain) {
        ...
        var self = this,
            ...
            onStart = function (response) {
                ...
                if (cookieJar && _.isFunction(cookieJar.getCookies)) {
                    const cookieJarOptions = { secure: isSpecialDomain(request.url.getHost()) ? true : null };

                    // get cookies set for the final request URL
                    cookieJar.getCookies(finalRequest.href, cookieJarOptions, function (err, cookiesFromJar) {
                        if (err) {
                            return done();
                        }
