...
var _ = require('lodash'),
     ...
     * @param {Number} [endTimer] - Optional cached end timer to avoid syscall
     * @returns {Object}
     */
    calcTimingsOffset = function (runtimeTimer, requestTimer, timings, endTimer) {
        ...
        if (offset.end) {
            // @note: Use cached endTimer if provided to reduce performance.now() syscall
            offset.done = (endTimer !== undefined ? endTimer : now()) - runtimeTimer;
        }
...
class Requester extends EventEmitter {
    ...
    request (id, request, protocolProfileBehavior, callback) {
        var self = this,
            ...
            getExecutionHistory = function (debugInfo, cachedEndTimer) {
                var history = {
                        execution: {
                ...
                debugInfo.forEach(function (debugData) {
                    ...
                    executionData.push({
                        ...
                        timings: debugData.timings && {
                            ...
                            // @note: Pass cached endTimer to reduce performance.now() syscall
                            offset: calcTimingsOffset(startTimer, debugData.timingStartTimer,
                                debugData.timings, cachedEndTimer)
                        },
        ...
        return requests(request, requestOptions, onStart, onData, function (err, res, resBody, debug) {
            // Cache end timer to avoid multiple performance.now() syscalls
            var endTimer = now(),
                endTime = Date.now(),
                history = getExecutionHistory(debug, endTimer),
                responseTime,
                response;
            ...
            // @note: Use cached endTime to avoid Date.now() syscall
