...
_.extend(Requester.prototype, /** @lends Requester.prototype */ {
     ...
     */
    request: function (item, sendId, cursor, callback) {
        var self = this,
        ...
        return requests(request, requestOptions, function (err, res, resBody) {
            if (err) {
                ...
                self.emit(sendId, err, self.trace.cursor, self.trace, undefined, request);
            ...
            self.emit(sendId, null, self.trace.cursor, self.trace, response, request, cookies);
