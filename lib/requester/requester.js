...
_.assign(Requester.prototype, /** @lends Requester.prototype */ {
    ...
    request: function (id, request, protocolProfileBehavior, callback) {
        ...
        return requests(request, requestOptions, function (err, res, resBody) {
            ...
            // raw request headers: GET / HTTP/1.1...\r\n\r\n
            rawRequestHeader = _.has(res, 'request.req._header') ? String(res.request.req._header) : '';

            // request body content length
            requestContentLength = _.has(res, 'request.headers.content-length') ?
                Number(res.request.headers['content-length']) : 0;

            requestSize = {
                header: Buffer.byteLength(rawRequestHeader),
                body: requestContentLength
            };

            requestSize.total = requestSize.header + requestSize.body;

            // update request.size, accessible in `request` and `response` callback
            request.update({size: requestSize});
